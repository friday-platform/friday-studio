...
export class AtlasLogger {
  ...
  private async writeLog(target: string, entry: any): Promise<void> {
    ...
    if (target === "global" && this.fileWriter) {
      ...
      if (!writer) {
        const workspacePath = join(this.logDir, "workspaces", `${workspaceId}.log`);
  ...
  private async log(level: string, message: string, context?: LogContext): Promise<void> {
    ...
    const reset = "\x1b[0m";
    const prefix = context ? `[${context.workerType || "main"}${context.workerId ? ":" + context.workerId.slice(0, 8) : ""}]` : "[main]";
    
    console.log(`${color}${entry.timestamp} ${level.toUpperCase()} ${prefix}${reset} ${message}`);
  ...
  async readLogs(target: string = "global", lines: number = 100): Promise<string[]> {
...
export class ChildLogger {
  ...
  async error(message: string, additionalContext?: LogContext): Promise<void> {
    await this.parent.error(message, { ...this.defaultContext, ...additionalContext });
  }
  
  async warn(message: string, additionalContext?: LogContext): Promise<void> {
    await this.parent.warn(message, { ...this.defaultContext, ...additionalContext });
  }
  
  async info(message: string, additionalContext?: LogContext): Promise<void> {
    await this.parent.info(message, { ...this.defaultContext, ...additionalContext });
  }
  
  async debug(message: string, additionalContext?: LogContext): Promise<void> {
    await this.parent.debug(message, { ...this.defaultContext, ...additionalContext });
  }
  
  async trace(message: string, additionalContext?: LogContext): Promise<void> {
    await this.parent.trace(message, { ...this.defaultContext, ...additionalContext });
  }
}
