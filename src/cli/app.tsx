...
export default function App({ command, subcommand, args, flags }: AppProps) {
  ...
  switch (command) {
    ...
    default:
      return <Text color="red">Unknown command: {command}. Run 'atlas help' for usage.</Text>;
  }
}
...
function HelpComponent() {
  return (
    <Box flexDirection="column">
      ...
      <Text color="gray">  session cancel &lt;id&gt;         Cancel a running session</Text>
      <Newline />
      <Text color="gray">  signal list                    List all signals</Text>
      <Text color="gray">  signal trigger &lt;name&gt; -d    Trigger a signal with data</Text>
      ...
      <Text color="gray">  atlas signal trigger telephone-message --data '{`{"message": "Hello"}`}'</Text>
