...
export class ParallelNode extends BaseNode {
  ...
  private evaluateResult(): NodeStatus {
    ...
    const successes = Array.from(this.childStatuses.values()).filter(s => s === NodeStatus.SUCCESS).length;
    const failures = Array.from(this.childStatuses.values()).filter(s => s === NodeStatus.FAILURE).length;
    const running = Array.from(this.childStatuses.values()).filter(s => s === NodeStatus.RUNNING).length;
    const total = this.children.length;
    
    this.log(`Parallel result: ${successes} successes, ${failures} failures, ${running} running out of ${total} total`);
  ...
  override validate(): { valid: boolean; errors: string[] } {
    ...
    if (config.policy === "threshold" && config.threshold && config.threshold > this.children.length) {
