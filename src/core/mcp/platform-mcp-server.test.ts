...
// Test suite
Deno.test("Platform MCP Server - Job Discoverability Filtering", async (t) => {
  ...
  try {
    ...
    await t.step("checkJobDiscoverable - wildcard match", async () => {
      ...
      assertEquals(isDiscoverable, true, "public_demo should be discoverable via public_* pattern");
    });
    ...
    await t.step("Empty discoverable jobs list blocks all jobs", async () => {
      ...
      assertEquals(isDiscoverable, false, "jobs should be blocked when discoverable list is empty");
    });
  } finally {
...
Deno.test("Platform MCP Server - Integration Test with Tool Calls", async (t) => {
    ...
    await t.step("workspace_jobs_list filters jobs correctly", async () => {
      ...
      const result = await (mcpServer as any).withWorkspaceMCPCheck(
        ...
        async ({ workspaceId }) => {
          ...
          const response = await fetch(`http://localhost:8082/api/workspaces/${workspaceId}/jobs`);
...
Deno.test("Platform MCP Server - Edge Cases", async (t) => {
