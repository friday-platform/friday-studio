...
Deno.test("Two-Level MCP Integration Tests", async (t) => {
  ...
  try {
    ...
    await t.step("Workspace-level job filtering with atlas.yml constraints", async () => {
      ...
      for (const { job, expected } of tests) {
        ...
        assertEquals(isDiscoverable, expected, `Job ${job} discoverability should be ${expected}`);
      }
    });
  } finally {
...
Deno.test("Two-Level MCP Error Handling", async (t) => {
