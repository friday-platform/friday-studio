...
export class MCPServerRegistry {
  ...
  private static extractPlatformMCPServers(
    ...
    if (!atlasConfig?.mcp_servers) return servers;

    for (const [serverId, config] of Object.entries(atlasConfig.mcp_servers)) {
  ...
  private static extractWorkspaceMCPServers(
    ...
    if (!workspaceConfig?.mcp_servers) return servers;

    for (const [serverId, config] of Object.entries(workspaceConfig.mcp_servers)) {
      servers.set(serverId, {
        ...config,
        id: serverId,
        timeout_ms: config.timeout_ms || 30000,
