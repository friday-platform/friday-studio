  ...
  return createMachine({
    ...
    states: {
      ...
      terminating: {
        ...
        entry: ({ context }) => {
          logger.info(`Terminating worker`, { workerId: context.id, workerType: context.type });
        }
      },
      terminated: {
        type: 'final'
...
export class WorkerManager {
      ...
      actions: {
        ...
        assignTask: ({ context, event }) => {
          if (event.type === 'ASSIGN_TASK') {
            ...
            const worker = this.workers.get(event.workerId);
            worker?.actor.send({ type: 'START_PROCESSING', taskId: event.taskId });
          }
        },
        completeTask: ({ context, event }) => {
          if (event.type === 'TASK_COMPLETED') {
            ...
            const worker = this.workers.get(event.workerId);
            worker?.actor.send({ type: 'COMPLETE_PROCESSING', taskId: event.taskId });
          }
        },
  ...
  async spawnWorker(metadata: WorkerMetadata, url: string): Promise<ManagedWorker> {
    ...
    worker.onmessage = (event) => {
      logger.trace(`Message from worker`, { workerId: id, messageType: event.data.type });
