...
import type { ProviderState, ProviderCredentials } from "./types.ts";
...
export class ProviderStateManager {
  ...
  async load(): Promise<void> {
    try {
      ...
      console.log(`[ProviderStateManager] Loaded state for ${Object.keys(this.state.providers).length} providers`);
    } catch (error) {
  ...
  async storeCredentials(providerId: string, credentials: ProviderCredentials): Promise<void> {
    ...
    const warning = "WARNING: Credentials stored in plain text. Use proper encryption in production!";
    
    await Deno.writeTextFile(credPath, JSON.stringify({
      warning,
      credentials
    }, null, 2));
  }
