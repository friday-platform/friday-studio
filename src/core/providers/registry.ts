...
export class ProviderRegistry implements IProviderRegistry {
  ...
  registerFactory(providerId: string, factory: (config: ProviderConfig) => Promise<IProvider>): void {
  ...
  static registerBuiltinProviders() {
    ...
    registry.registerFactory('anthropic', async (config) => {
      const { AnthropicAgentProvider } = await import('./builtin/anthropic-agent.ts');
