...
export class WorkspaceLoader {
  ...
  async load(): Promise<IWorkspace> {
    ...
    if (this.config.workflows) {
      for (const workflowConfig of this.config.workflows) {
        ...
        console.log(`[WorkspaceLoader] TODO: Load workflow ${workflowConfig.id}`);
      }
    }
  ...
  private async loadProviders(): Promise<void> {
    ...
    console.log(`[WorkspaceLoader] Loading ${this.config.providers.length} providers`);
    ...
    for (const providerConfig of this.config.providers) {
      try {
        ...
        if (savedState) {
          console.log(`[WorkspaceLoader] Found saved state for provider ${provider.id}`);
        ...
        console.log(`[WorkspaceLoader] Provider ${provider.id} loaded and ready`);
        
      } catch (error) {
        console.error(`[WorkspaceLoader] Failed to load provider ${providerConfig.id}:`, error);
      }
    }
  }
  ...
  private async loadSignals(workspace: IWorkspace): Promise<void> {
    ...
    console.log(`[WorkspaceLoader] Loading ${this.config.signals.length} signals`);
    ...
    for (const signalConfig of this.config.signals) {
      try {
        ...
        if (provider.type !== ProviderType.SIGNAL) {
          throw new Error(`Provider ${signalConfig.providerId} is not a signal provider`);
        }
        ...
        console.error(`[WorkspaceLoader] Failed to load signal ${signalConfig.id}:`, error);
      }
    }
  }
  ...
  private async loadAgents(workspace: IWorkspace): Promise<void> {
    ...
    console.log(`[WorkspaceLoader] Loading ${this.config.agents.length} agents`);
    ...
    for (const agentConfig of this.config.agents) {
      try {
        ...
        if (provider.type !== ProviderType.AGENT) {
          throw new Error(`Provider ${agentConfig.providerId} is not an agent provider`);
        }
        
        // For now, just log - agent creation is handled by AgentRegistry
        console.log(`[WorkspaceLoader] Agent ${agentConfig.id} configured with provider ${agentConfig.providerId}`);
        ...
        console.error(`[WorkspaceLoader] Failed to configure agent ${agentConfig.id}:`, error);
      }
    }
  }
