...
```typescript
// ❌ types.test.ts - EVERY test here just verifies Zod/TypeScript work
...
```typescript
// ✅ Test YOUR code's behavior, not the library
...
// ✅ Test real component or don't assert on mock
test("renders sidebar", () => {
  render(<Page />);
...
```typescript
// ❌ Mock breaks test - it prevented config write test depends on
...
```typescript
// ❌ Partial mock - missing fields downstream code uses
const mockResponse = {
  status: "success",
  data: { userId: "123" },
