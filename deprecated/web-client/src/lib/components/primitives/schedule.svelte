<script lang="ts">
  import { CalendarScheduleSchema, type CalendarSchedule } from "@atlas/core/artifacts";
  ...
  const hasAllDayEvents = $derived(
    Array.from(parsedEvents.values()).some((event) => event.isAllDay),
  );
