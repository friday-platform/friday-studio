<!-- v3 - 2026-04-26 - Generated via /improving-plans from docs/plans/2026-04-26-webhook-tunnel-go-rewrite-design.v2.md -->
...
10. As an atlas dev, I want webhook-tunnel to reject malformed or
...
18. As a webhook integrator debugging signature failures, I want the same
...
19. As an oncall during a tunnel outage, I want the auto-reconnect with
...
20. As an oncall, I want the two-axis health probe (process alive + edge
...
21. As an oncall stopping the launcher, I want webhook-tunnel to drain
...
22. As a launcher author, I want webhook-tunnel's binary to live in the
...
23. As a future Go-binary author, I want a stable `processkit` API so my
...
- **Interface:** `Resolve(ctx) → (path string, error)`. Implements the
  four-tier discovery + sidecar-verified download fallback described above.
- **Hides:** Discovery order; the per-platform cloudflared release URL
  ...
  under `~/.atlas/bin/`; download progress reporting; concurrent-call
  coalescing (if two goroutines call `Resolve` simultaneously on a
  missing-binary machine, only one download fires).
- **Trust contract:** Caller asks for "a usable cloudflared binary" and
  gets back a path it can hand to `exec.Command`. The pinned cloudflared
...
- **Interface:** `New(component string) *Logger`; level methods
  ...
  Mirrors TS `@atlas/logger` shape so engineers switching between the TS
  and Go sides see the same idiom.
