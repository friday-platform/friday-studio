<!-- v14 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v13.md -->

# Installer + Launcher UX Fixes (v14)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v13 (`2026-04-27-installer-launcher-ux-fixes.v13.md`)

> **What v14 changes from v13 (real design changes, not polish):**
> Two fixes for gaps that survived all 13 prior reviews — both
> small, both real:
> 1. **Migration respects a previously-disabled autostart.**
>    v13's migration step 9 unconditionally re-registered
>    autostart. If the user had explicitly disabled it on
>    v0.0.8 (`friday-launcher --autostart disable`), the
>    upgrade silently re-enabled it — overriding the user's
>    choice. v14 captures the prior plist's existence in step 3
>    and conditionally skips step 9 if autostart was previously
>    absent.
> 2. **Migration backup moves to `~/.friday/local/backup-<pid>`.**
>    v13 used `/tmp/friday-app-backup-<pid>` to keep the orphan
>    out of LaunchServices's index path. But macOS `/tmp` is
>    shared across users — a multi-user machine could see one
>    user's pre-extract glob sweep wipe another user's backup.
>    `~/.friday/local/` is per-user by definition, also outside
>    LaunchServices's index path, and matches the existing
>    install-paths convention. Pre-extract cleanup glob updated
>    accordingly.
>
> All v13 design decisions roll forward unchanged: Info.plist
> assertion (#35), CAS gate in `performShutdown` (#33), startup
> error log under `~/.friday/local/logs/` (#34), `open -b`
> bundle ID (#29). Plus everything earlier.
...
36 numbered decisions (§Decisions); 14 risk callouts (§Risks);
...
func handleShutdown(cache *HealthCache) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        // Probe the gate so concurrent HTTP callers see 409.
        // The actual shutdown gate lives in performShutdown
        // itself (Decision #33); this only serves the HTTP
        // contract.
        if cache.shuttingDown.Load() {
...
// performShutdown is the single choke point for ALL shutdown
// triggers (tray Quit, NSApp will-terminate, HTTP POST). The
// CAS gate here makes the function idempotent — concurrent
// callers all return after the first one wins. (Decision #33.)
func performShutdown(reason string) {
    if !cache.shuttingDown.CompareAndSwap(false, true) {
        log.Printf("performShutdown(%s): already in progress, skipping", reason)
        return
    }
    log.Printf("performShutdown(%s): begin", reason)
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v14.md`
  § Migration.
...
## Decisions (confirmed 2026-04-27, v14 pass)
...
33. **`performShutdown` is the single CAS-gated choke point for
    all shutdown triggers** (NEW in v12) — v11's
    `cache.shuttingDown.CompareAndSwap` lived inside
    `handleShutdown` (the HTTP POST handler). That gated only
    the HTTP path; tray-Quit, NSApp will-terminate, and
    `--uninstall`'s in-process call all spawned `performShutdown`
    independently with no atomic gate. Two simultaneous triggers
    (e.g. user clicks tray Quit while installer's update flow
    POSTs `/api/launcher-shutdown`) would each call
    `supervisor.Shutdown`, `SweepByBinaryPath`, and
    `srv.Shutdown(ctx)` concurrently — most operations are
    idempotent in practice but `srv.Shutdown` returns
    ErrServerClosed on the second call, polluting logs and
    confusing post-mortems. v12 hoists the CAS gate into
    `performShutdown` itself; the function returns early if
    shutdown is already in progress. The HTTP handler still does
    a `Load()` probe to return 409 to concurrent HTTP callers,
    but the actual shutdown work is gated function-level.
    Idempotent at the level that matters; trigger paths can call
    `performShutdown(reason)` from anywhere without coordinating.
