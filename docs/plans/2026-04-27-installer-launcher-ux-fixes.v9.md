<!-- v9 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v8.md -->

# Installer + Launcher UX Fixes (v9)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v8 (`2026-04-27-installer-launcher-ux-fixes.v8.md`)

> **What v9 changes from v8 (real design changes, not polish):**
> Five gap-fixes, all motivated by real edge cases the v8 design
> would hit at runtime:
> 1. **LICENSE files come from pinned GitHub raw URLs**, not
>    extracted from upstream release archives. Cloudflared on
>    Windows ships as a bare `.exe` (no archive), so the v8
>    "extract LICENSE from archive root" approach was broken on
>    Windows. Same-source-of-truth across all platforms.
> 2. **Port 5199 bind-failure shows an osascript dialog + exits**
>    instead of vanishing silently. Same dialog mechanism as
>    pre-flight for missing binaries — symmetric UX for the two
>    "launcher can't start" failure modes.
> 3. **LaunchAgent autostart runs `open -a "Friday Studio"`**, not
>    the bare `Contents/MacOS/friday-launcher` binary. Goes through
>    LaunchServices so Info.plist (LSUIElement, env, icon) is
>    consulted on every boot, matching the user-launched path.
> 4. **Migration extracts to `bin.new/` staging dir + atomic swap.**
>    v8's order (remove-old-then-extract) leaves an unbootable
>    system if extract fails after binary removal. New order:
>    extract → swap → remove old (the post-swap remove is
>    fully recoverable from re-running extract).
> 5. **`wait_health.rs` SSE backoff deadline 10s → 20s.** Tight
>    on slow Macs where `open -a` LaunchServices spin-up + cold
>    Mach-O load + supervisor init can brush 6-8s. Doubling the
>    budget adds zero common-case latency (first retry succeeds
>    in ~200ms regardless).
...
30 numbered decisions (§Decisions); 13 risk callouts (§Risks);
6 verification items to check on test hardware during
implementation.
...
v9 uses an extract-to-staging then atomic-swap pattern (Decision
#30) so that any failure mid-extract is fully recoverable from
...
3. **Disable old autostart.** The LaunchAgent label
   ...
   plist file itself will be overwritten in step 7 with the new
   `open -a` invocation.
4. **Extract new tarball with split-destination INTO STAGING.**
   ...
   `.new` dirs, surface the error to the wizard, and exit.
   The user's existing v0.0.8 install is **untouched** — they
   can keep using it while debugging the failed install.
5. **Atomic swap.** Once extraction completes successfully:
   ...
   layout is live.
6. **Clean up old flat-layout binaries** from the v0.0.8 paths.
   Now safe because the new layout is already live in step 5:
...
9. **First launch.** Spawn the new launcher via `open -a "Friday
...
- **Negative test:** force the extract to fail (e.g. `chmod -w
  /Applications` mid-extract); verify the user's v0.0.8 install is
  untouched (binaries still present, autostart still works after
  `launchctl load`); verify no `.new` cruft left behind; re-running
  the installer succeeds without manual cleanup.
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  to .app + binaries split. Uses extract-to-staging then atomic
  swap so a failed extract leaves the v0.0.8 install intact.
  See `docs/plans/2026-04-27-installer-launcher-ux-fixes.v9.md`
  § Migration.
...
- **Migration is one-shot but extract is recoverable**: v9's
  staging-then-atomic-swap pattern (Decision #30) means a failed
  extract leaves the user's existing v0.0.8 install intact and
  no half-installed state on disk. The remaining failure mode is
  step 8 (re-register autostart): if the new plist write fails,
  the .app is in place but autostart is missing. Recovery: run
  the installer again or invoke `friday-launcher --autostart
  enable` manually.
...
| 3 | `scripts/build-studio.ts` (new test or build-time assertion) | LICENSE files present at expected output paths after build; each file is non-empty; SHA-256 matches the pinned upstream URL's expected hash. |
| 3 | Manual QA on a v0.0.8-installed machine | End-to-end migration per the §Test before shipping checklist (LaunchAgent path, Spotlight indexing, silent first launch, reboot autostart, **negative-path test** of mid-extract failure). |
...
## Decisions (confirmed 2026-04-27, v9 pass)
...
27. **License files come from pinned GitHub raw URLs, not
    upstream release archives** (NEW in v9) — v8 specified
    "extract LICENSE from the same release archive we already
    download for the binary". That fails on Windows cloudflared,
    which is shipped as a bare `.exe` (no archive at all). Even
    where archives exist, the path of the LICENSE file inside
    them isn't stable across upstream releases. v9 instead
    downloads each LICENSE once per build from a pinned commit
    SHA on the upstream repo's GitHub raw URL:
    - `https://raw.githubusercontent.com/nats-io/nats-server/v<version>/LICENSE`
    - `https://raw.githubusercontent.com/cloudflare/cloudflared/<version>/LICENSE`
    - `https://raw.githubusercontent.com/cli/cli/v<version>/LICENSE`
    The pin is on the same version tag as the binary, so license
    text matches the binary version. Pin a specific tag (not
    `master`) so a future force-push to upstream's main branch
    can't change the license text underneath us. Build asserts
    each LICENSE file is non-empty post-download; otherwise fails
    loudly. Generate a `bin/LICENSE-CHECKLIST.md` at build time
    listing each license + its source URL for human audit before
    release.
...
29. **LaunchAgent autostart targets `open -a "Friday Studio"`,
    ...
    — going through LaunchServices means Info.plist (LSUIElement,
    env, icon, quarantine flags) is consulted on every boot,
    matching the user-launched path via Spotlight. Direct exec
    of the bundle's binary skips LaunchServices, which would
    create subtle dev/prod parity bugs (e.g. an LSUIElement
    update only takes effect after the user re-launches via
    Finder). The plist's `ProgramArguments` becomes
    `["/usr/bin/open", "-a", "Friday Studio", "--args",
    "--no-browser"]`. Trade-off: launchd's child is `open`, not
    the launcher itself, but the launcher detaches anyway so
    process-tree semantics are unchanged.
