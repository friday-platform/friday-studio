<!-- v7 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v6.md -->

# Installer + Launcher UX Fixes (v7)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready (design converged after 5 passes; v7 is document polish only)
**Supersedes:** v6 (`2026-04-27-installer-launcher-ux-fixes.v6.md`)

> **Convergence note:** v7 adds no new design content. Three
> document-organization improvements over v6: a TLDR up top, an
> explicit Recommended Implementation Order section, and a Test
> Matrix table. Both nail down information that has been correct
> across the last several review cycles but only existed in chat
> or scattered through the file list. Future review cycles
> would be gold-plating; the next step is to write the code.
...
3. **Friday Studio.app for Spotlight.** macOS tarball ships
   `Friday Studio.app` (bundle id `ai.hellofriday.studio-launcher`)
   that lands in `/Applications`. Quarantine xattr stripped
   post-extract for silent first-launch. Pre-flight check via
   osascript dialog if supervised binaries are missing.
...
6. **Build pipeline.** `studio-build.yml` emits a backwards-
   compatible v0.0.9 tarball (.app + duplicate flat
   `friday-launcher` at root for v0.1.15 cutover);
   `studio-installer-build.yml` emits v0.1.16 with the wizard
   rewrite + migration. Rollout-order-agnostic during the
   transition.

24 numbered decisions (§Decisions); 11 risk callouts (§Risks);
...
6. **Launcher's `binDir` resolution updated for .app context.**
...
- **macOS install paths**: `Friday Studio.app` is in
  `/Applications`; supervised binaries are in `~/.friday/local/`;
  pids are `~/.friday/local/pids/`; logs are
  `~/.friday/local/logs/`.
...
5. **Test order:** ship v0.0.9 platform first (existing v0.1.15
   ...
   to /Applications + binaries to ~/.friday/local → strips
   quarantine xattr → launcher exposes /api/launcher-health →
   wizard's wait step subscribes with capped-backoff retry +
   staged 60s/90s/+60s deadline → tray bucket reads health-cache
   → Spotlight finds the .app → silent first `open -a` → Quit
   tears it all down, `--uninstall` sweeps every orphan.
