<!-- v11 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v10.md -->

# Installer + Launcher UX Fixes (v11)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v10 (`2026-04-27-installer-launcher-ux-fixes.v10.md`)

> **What v11 changes from v10 (real design changes, not polish):**
> Three small spec-tightenings that close concrete implementer
> ambiguities:
> 1. **HTTP shutdown POST response table.** v10 said "POST first,
>    fall back to SIGTERM" without specifying which HTTP outcomes
>    fall through. v11 specifies the full response → action
>    table (connection-refused, 202, 409, 4xx/5xx, timeout) so
>    two implementers don't pick two different policies.
> 2. **Decision #32 caveat for auth-walled probe paths.** v10
>    said probes target user-facing paths, not sidecar `/health`.
>    Today playground's `/` is public; future services with auth
>    walls would never go `healthy` under v10's contract. v11
>    refines: probe a *publicly-reachable* path that exercises
>    the same handler stack as the user-loaded surface — for
>    auth-walled services, the public landing or login page,
>    NOT a sidecar.
> 3. **Pre-extract cleanup also wipes `Friday Studio.app.bak`.**
>    Decision #31's pre-extract cleanup covered `bin.new/` and
>    `Friday Studio.app.new` but not the `.bak` orphan migration
>    step 6 creates. A crash between step 6 and step 7 could
>    leave `.bak` hanging around with Spotlight indexing both
>    .app paths. One-line fix.
>
> All v10 design decisions roll forward unchanged: pre-extract
> cleanup of `.new` dirs (#31), `open -a` arg-drop limitation
> (#29), license URL version interpolation (#27), readiness
> probes target user-facing paths (#32). Plus everything from
> v9: pinned LICENSE URLs, port-5199 dialog, autostart via
> `open -a`, staging-then-swap migration, 20s SSE-connect
> deadline.
...
implementation. (v11 tightens existing decisions; no new
decisions added.)
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v11.md`
  § Migration.
...
## Decisions (confirmed 2026-04-27, v11 pass)
