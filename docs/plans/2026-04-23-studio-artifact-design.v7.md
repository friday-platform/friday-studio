<!-- v7 - 2026-04-24 - Generated via /improving-plans from docs/plans/2026-04-23-studio-artifact-design.v6.md -->
...
7. As an operator, I want the service to expose a `/healthz` endpoint, so that Kubernetes liveness and readiness probes work without authentication.
...
- `GET /healthz` — handled by `middleware.Heartbeat("/healthz")`, excluded from request logs
...
| Clamped sub-range | `Range: bytes=N-M` where M ≥ size | `206 Partial Content`, `Content-Range: bytes N-{size-1}/{size}` |
...
**Resolution:** The download port's `http.Server` is constructed directly in
`main.go`, following the cortex pattern exactly:

```go
// SetupTLS must run before constructing the server
if err := cfg.TLSConfig.SetupTLS(); err != nil {
    log.Fatal("TLS setup failed", "error", err)
}
...
httpServer := &http.Server{
    ...
    WriteTimeout:      0, // disabled — large file streaming
}

// Start: branch on TLS like cortex does
go func() {
...
`WriteTimeout: 0` is required — do not use `pkg/server`'s `Listen()` for the
download port, as it defaults `WriteTimeout: 0` to 30 seconds. `ReadHeaderTimeout: 10s`
defends against slow-loris attacks on the request side. `SetupTLS()` initialises
the TLS config from cert/key files; without it `GetTLSConfig()` returns nil even
when cert paths are set.
...
// ObjectMeta holds the GCS object attributes needed by the handler.
type ObjectMeta struct {
    ETag         string    // from storage.ObjectAttrs.Etag — valid HTTP ETag string
    Size         int64     // from storage.ObjectAttrs.Size — total object bytes
    ContentType  string    // from storage.ObjectAttrs.ContentType
    LastModified time.Time // from storage.ObjectAttrs.Updated
}
```

`Config` parsed with `github.com/caarlos0/env/v11`. `.env` file loaded via
`godotenv` if `DOT_ENV` is set or `.env` exists.
...
- **Hides:** GCS bucket name, `NewRangeReader` call convention (`length=-1` sentinel for "to end"), GCS error type translation to `ErrNotFound`, mapping `storage.ObjectAttrs` fields to `ObjectMeta`
...
- **Hides:** leading-slash stripping (`strings.TrimPrefix(r.URL.Path, "/")`), RFC 7233 range header parsing (including range clamping: for `bytes=N-M`, `end = min(M, size-1)` when M ≥ size — `Content-Range` reflects the actual end byte, not the requested M), precondition evaluation order (If-None-Match / If-Modified-Since evaluated before Range per RFC 7233 §6 — a matching ETag returns 304 even if a Range header is present), `If-Modified-Since` precision handling (compare `meta.LastModified.Truncate(time.Second)` — GCS timestamps have nanosecond precision but HTTP headers are second-precision; without truncation every conditional GET returns 200 instead of 304), HEAD short-circuit, multi-range rejection (respond `200`), Cache-Control selection, all response header assembly
- **Trust contract:** Caller registers one handler. It always produces a valid HTTP response. HEAD uses only `Stat`. GET streams via `io.Copy` without buffering. Non-ErrNotFound errors from `Stat`/`Open` are logged at error level (with the underlying error) before returning 500.
...
apps/studio-artifact/
  main.go              — env load → config parse → service.New → SetupTLS → metrics → http.Server (WriteTimeout:0, ReadHeaderTimeout:10s, TLSConfig) → ListenAndServe[TLS] → signal wait → graceful shutdown (30s)
  service/
    service.go         — Config, ObjectMeta, Service struct, New(), Router(), Close()
    storage.go         — StorageClient: Stat(), Open(); ObjectMeta from attrs; path must not start with /
    handler.go         — ServeObject http.HandlerFunc (slash-strip, precondition eval + time truncation, range parse+clamp, HEAD short-circuit, Cache-Control, streaming, error logging)
  Dockerfile
```
...
- `middleware.Heartbeat("/healthz")` for health probe (excluded from request logs)
...
```dockerfile
FROM golang:1.26.2-alpine3.23 AS build-env
...
RUN CGO_ENABLED=0 GOOS=linux go build \
    -ldflags="-X main.GitCommit=${GITHUB_SHA}" \
    -o /go/bin/studio-artifact ./apps/studio-artifact

FROM alpine:3.23.3
RUN apk --no-cache add ca-certificates
COPY --from=build-env /go/bin/studio-artifact /studio-artifact
USER 65534:65534
ENTRYPOINT ["/studio-artifact"]
```

`ca-certificates` is required in the runtime stage — alpine ships without root
CA certificates, and without them the GCS HTTPS client fails with
`x509: certificate signed by unknown authority`. Both cortex and gist include
this step.
...
- **Unit tests on `handler.go`**: inject a `fakeStorage` that returns canned
  ...
  and body bytes for: full download, range request, resume, invalid range,
  clamped range (M ≥ size — assert `Content-Range` end = size-1 not M),
  not found, conditional GET hit/miss (`If-None-Match` matching `ETag`,
  `If-Modified-Since` — use a `LastModified` with sub-second precision to
  confirm truncation works), conditional GET + Range (assert 304 returned,
  not 206), HEAD (assert no GCS `Open` call, correct headers present including
  `ETag` from `ObjectMeta`).
...
- **Suffix range requests** (`Range: bytes=-N`) — responded to with `200 OK`
  and full body; the installer never uses suffix ranges (it always uses
  `bytes={received}-` for resume).
