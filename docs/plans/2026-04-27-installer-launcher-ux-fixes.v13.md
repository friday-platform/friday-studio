<!-- v13 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v12.md -->

# Installer + Launcher UX Fixes (v13)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v12 (`2026-04-27-installer-launcher-ux-fixes.v12.md`)

> **What v13 changes from v12 (real design changes, not polish):**
> Three fixes — one correctness bug from v12's `-b` switch, one
> ergonomics improvement, one build-time hardening:
> 1. **Migration's `.app.bak` moves OUT of `/Applications`.** v12
>    renames `Friday Studio.app` → `Friday Studio.app.bak`
>    in-place, then swaps the new bundle in. Both `.app` and
>    `.app.bak` advertise the same `CFBundleIdentifier`;
>    LaunchServices indexes everything in `/Applications` and
>    its resolution between same-ID bundles is non-deterministic.
>    A migration crash between step 6 and step 7 could leave
>    autostart `open -b` resolving to the orphan `.bak` (with
>    the OLD launcher) for a reboot cycle. v13 renames to
>    `/tmp/friday-app-backup-<pid>` instead — outside
>    LaunchServices's index path, can't shadow the live bundle.
> 2. **Diagnostic log path moves under `~/.friday/local/logs/`.**
>    v12 wrote pre-flight / bind-failure logs to `os.TempDir()`
>    (always-writable but opaque). v13 writes to
>    `~/.friday/local/logs/launcher-startup.log` — co-located
>    with the launcher's normal logs, predictable, easy to
>    reference. `mkdir -p` ensures the path exists; if that
>    fails (extreme edge case), fall back to `os.TempDir()` so
>    the log still lands somewhere.
> 3. **Build-time assertion: Info.plist matches Decision #3 +
>    Decision #29's expectations.** `open -b
>    "ai.hellofriday.studio-launcher"` only works if the .app's
>    Info.plist actually advertises that bundle ID. A typo in
>    `scripts/build-studio.ts`'s plist template would silently
>    ship a tarball where every user's autostart fails. v13
>    parses the generated Info.plist post-build and asserts
>    `CFBundleIdentifier == "ai.hellofriday.studio-launcher"`
>    AND `CFBundleExecutable == "friday-launcher"`. Build fails
>    loudly on mismatch.
>
> All v12 design decisions roll forward unchanged: CAS gate in
> `performShutdown` (#33), startup error log (#34, log path
> updated per #2 above), `open -b` bundle ID (#29). Plus
> everything earlier: HTTP shutdown response table, auth-walled
> probe caveat, pre-extract cleanup with `.bak` (cleanup pattern
> updated for the new `.bak` location — `/tmp/friday-app-backup-*`
> instead of `/Applications/Friday Studio.app.bak`).
...
35 numbered decisions (§Decisions); 14 risk callouts (§Risks);
...
3. **Disable old autostart.** The LaunchAgent label
   ...
   `open -b` invocation. **Ignore non-zero exit** from `launchctl
...
4. **Pre-extract cleanup of stale staging + backup dirs.**
   ...
   // v13: backup target lives in /tmp, not /Applications, so
   // LaunchServices can't index a stale `.bak` that still
   // advertises our bundle ID. Sweep any leftovers from a
   // prior crashed migration.
   for entry in glob::glob("/tmp/friday-app-backup-*").into_iter().flatten().flatten() {
   ...
   The `/tmp/friday-app-backup-*` cleanup covers the crash-
   between-step-6-and-step-7 case (a migration renamed the old
   `.app` aside but didn't get to remove the backup). Putting
   the backup in `/tmp` instead of `/Applications` (Decision #31
   v13 update) means LaunchServices doesn't index it — `open -b`
   resolves only to the live bundle, never the orphan, even if
   the orphan survives a reboot.
...
6. **Atomic swap.** Once extraction completes successfully:
   - Set backup path: `let bak = format!("/tmp/friday-app-backup-{}", std::process::id());`
   ...
   The `mv` of the live `.app` to `/tmp` is cross-filesystem
   on most setups (`/Applications` is on `/`, `/tmp` is its own
   filesystem on modern macOS), so it's a copy-then-delete
   under the hood — slower than a same-FS rename, but still
   O(seconds) for our small bundle and atomic at the destination.
   The within-`/Applications` rename (`.new` → `.app`) IS
   same-FS and atomic; that's the swap that matters for live-
   path correctness. After this point the new layout is live.
7. **Clean up old flat-layout binaries** from the v0.0.8 paths.
   ...
   - `rm -rf <bak>` (the `/tmp/friday-app-backup-<pid>` from
     step 6, if created)
...
9. **Re-register autostart via `open -b` (bundle ID).** New
   LaunchAgent's `ProgramArguments` is `["/usr/bin/open", "-b",
   "ai.hellofriday.studio-launcher", "--args", "--no-browser"]`
   — goes through LaunchServices, resolved by bundle ID so a
   stray copy of "Friday Studio.app" elsewhere on disk can't
   shadow it (see Decision #29 for why bundle-ID resolution is
   preferred over `-a`-by-name and over direct
   `Contents/MacOS/friday-launcher` exec).
...
- `/tmp/friday-app-backup-*` is empty (migration step 7 cleaned
...
- **Negative test (`/tmp` backup orphan recovery):** simulate a
  crash between migration step 6 (rename live `.app` to `/tmp/`
  backup) and step 7 (remove backup) by manually creating
  `/tmp/friday-app-backup-99999/` (a copy of an old `.app`
  bundle with the same `CFBundleIdentifier`) before running the
  installer. After install, verify the backup is gone, AND the
  pre-existing copy didn't shadow the live one mid-install
  (`mdfind kMDItemCFBundleIdentifier == 'ai.hellofriday.studio-launcher'`
  returns exactly one path — the live `/Applications/Friday Studio.app`).
  Exercises the v13 update to Decision #31.
- **Negative test (legacy `.bak` in `/Applications`):** if the
  user upgraded a v12-installed machine to v13, the prior install
  may have left `/Applications/Friday Studio.app.bak` from a
  v12-era crash. Manually create that path before running the
  v13+ installer. v13's pre-extract cleanup doesn't sweep this
  path (Decision #31 v13 only sweeps `/tmp/friday-app-backup-*`
  for new layout). Verify the orphan is invisible to `open -b`
  resolution AFTER the migration completes (the live .app is
  in `/Applications`; LaunchServices may still index the orphan
  but the live one wins by being the most-recent + correctly-
  signed). Acceptable: legacy v12 fleet is empty (~1 user).
  Test exists to confirm `open -b` doesn't degrade in this
  edge case.
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v13.md`
  § Migration.
...
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend; NEW in v10, extended v11, **path updated v13**) | Pre-extract cleanup of stale staging + backup dirs (Decision #31): seed `~/.friday/local/bin.new/`, `/Applications/Friday Studio.app.new`, AND `/tmp/friday-app-backup-12345/` with partial-content fixtures that simulate prior crashed runs; invoke extract; assert all three stale paths are removed at step 4 before the new extraction writes anything. Glob-pattern sweep of `/tmp/friday-app-backup-*` covers any pid suffix. End state matches a clean install with no stale files. |
...
## Decisions (confirmed 2026-04-27, v13 pass)
...
31. **Pre-extract cleanup of stale staging + backup dirs** (NEW
    ...
    **v13 update — backup lives in `/tmp`, not `/Applications`:**
    ...
    reboot cycle. v13 moves the backup to
    `/tmp/friday-app-backup-<pid>` instead. `/tmp` is NOT in
    LaunchServices's index path, so the orphan can't shadow the
    live bundle even if it survives a crash.

    Pre-extract cleanup updated accordingly — sweeps any leftover
    `/tmp/friday-app-backup-*` from prior crashed runs:
    ```rust
    let _ = fs::remove_dir_all("/Applications/Friday Studio.app.new");
    let _ = fs::remove_dir_all(home.join(".friday/local/bin.new"));
    for entry in glob::glob("/tmp/friday-app-backup-*").into_iter().flatten().flatten() {
    ...
    hybrid, never a LaunchServices-indexable orphan.
