<!-- v15 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v14.md -->

# Installer + Launcher UX Fixes (v15)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v14 (`2026-04-27-installer-launcher-ux-fixes.v14.md`)

> **What v15 changes from v14 (alignment to current repo shape,
> not new design):** post-cleanup the codebase has scaffolding
> that earlier plan revisions didn't acknowledge. v15 brings the
> plan in line with what's actually on disk so implementers
> don't write duplicate code or diverge from established
> patterns:
> 1. **Use the existing `installed_marker.rs`.** The repo
>    already has `apps/studio-installer/src-tauri/src/commands/installed_marker.rs`
>    that writes `~/.friday/local/.installed` with `{ version,
>    installed_at }` JSON. The migration writes this marker on
>    completion (new step 11); future migrations dispatch on it
>    instead of inventing per-version detection heuristics. The
>    v0.0.8 → v0.0.9 migration still uses Mach-O sniff
>    (v0.0.8 predates the marker), but v0.0.9 → v0.1.x and
>    beyond can simplify.
> 2. **Decision #33 aligns with the existing two-atomic
>    pattern.** `tools/friday-launcher/main.go` already has
>    `shutdownStarted` (one-shot CAS gate) AND `shuttingDown`
>    (visibility flag). v14's pseudocode conflated them. v15
>    matches the existing pattern: HTTP handler does
>    `shuttingDown.Load()` for the 409 probe;
>    `performShutdown` does `shutdownStarted.CompareAndSwap`
>    (which the existing code ALREADY does — Decision #33 is
>    effectively "the existing pattern already covers concurrent
>    triggers; just don't add a redundant CAS in the HTTP
>    handler").
> 3. **Acknowledge `launch.rs` already resolves .app paths.**
>    `apps/studio-installer/src-tauri/src/commands/launch.rs`
>    already prefers `Friday Studio.app/Contents/MacOS/friday-launcher`
>    with a flat-bundle fallback. The implementer's work for
>    Issue 5 is on the **build-script side** (emitting the .app);
>    the **consumer side** is done. Plan adds a clarifying note
>    so an implementer doesn't rewrite resolution that already
>    exists.
> 4. **`extract.rs` migration work is a rewrite, not an
>    extension.** Current `extract.rs` does whole-install-dir
>    `.bak` rollback (`dest_path.with_extension("bak")`) +
>    `terminate_studio_processes` (SIGTERM-and-poll). v15's
>    split-destination + staging + atomic-swap migration
>    replaces both. The §Files to change line is updated to be
>    explicit: existing `extract_archive` and
>    `terminate_studio_processes` are removed, not extended.
>
> All v14 design decisions roll forward unchanged: autostart
> preference preservation (#36), per-user backup path (#31),
> Info.plist assertion (#35), startup error log under
> `~/.friday/local/logs/` (#34), CAS-in-`performShutdown`
> (#33), `open -b` bundle ID (#29). Plus everything earlier.
...
must land before Stack 2 (wizard consumes Stack 1's SSE endpoint)
and before Stack 3 (.app + migration depends on Stack 1's HTTP
shutdown POST).

### Stack 1 — Launcher HTTP server + Issue 6 (Quit/uninstall sweep)

**Decisions covered:**
- [x] #2  Quit confirmation modal
- [x] #4  Launcher exposes `/api/launcher-health` (HTTP + SSE)
- [x] #5  Drop Setpgid; rely on `processkit.SweepByBinaryPath`
- [x] #9  `--uninstall` always runs `SweepByBinaryPath`
- [x] #10 POST `/api/launcher-shutdown` returns 202 + async
- [x] #13 Cmd+Q does full performShutdown synchronously (NSApp will-terminate observer)
- [x] #18 HTTP server lifecycle decoupled from `Restart all`
- [ ] #21 Pre-flight only gates the normal-startup path (Stack 3 — pre-flight is binaries check)
- [x] #22 CLAUDE.md gets a Friday Studio platform layout section
- [x] #24 Orphan-sweep integration test uses a real binary
- [x] #28 Port 5199 bind-failure shows osascript dialog + exits
- [x] #33 `performShutdown` uses existing two-atomic CAS pattern
- [x] #34 Pre-flight + bind-failure write diagnostic log

**Files (new):**
- [x] `tools/friday-launcher/healthsvc.go`
- [x] `tools/friday-launcher/healthsvc_test.go`
- [x] `tools/friday-launcher/preflight_dialog_darwin.go` (port-in-use variant only; missing-binaries variant in Stack 3)
- [x] `tools/friday-launcher/preflight_dialog_windows.go` (port-in-use variant only)
- [x] `tools/friday-launcher/preflight_dialog_other.go` (Linux stub)
- [x] `tools/friday-launcher/preflight_dialog_test.go`
- [x] `tools/friday-launcher/confirm_darwin.go` + `confirm_darwin.m`
- [x] `tools/friday-launcher/confirm_windows.go`
- [x] `tools/friday-launcher/confirm_other.go` (Linux stub)
- [x] `tools/friday-launcher/nsapp_darwin.go` + `nsapp_darwin.m`
- [x] `tools/friday-launcher/nsapp_other.go` (Linux stub)
- [x] `tools/friday-launcher/tray_test.go`
- [x] `tools/friday-launcher/uninstall_test.go`

**Files (edit):**
- [x] `tools/friday-launcher/main.go` — start/close health server, NSApp will-terminate hook, bind-failure → dialog, `--no-browser` semantic refinement
- [x] `tools/friday-launcher/tray.go` — bucket logic reads health-cache, Quit confirmation modal, `--no-browser` semantic refinement
- [x] `tools/friday-launcher/uninstall.go` — try HTTP shutdown, fall back to SIGTERM, unconditional `SweepByBinaryPath`
- [x] `tools/friday-launcher/openers.go` — `openURLInBrowserOverride` test hook
- [x] `tools/friday-launcher/integration_test.go` — orphan-sweep test with real stub binary
- [x] `CLAUDE.md` — Friday Studio platform-layout section

**Live macOS QA (coordinate with user):**
- [ ] Tray bucket goes green when all 6 services report healthy (no more "Starting…" stuck)
- [ ] Quit menu shows confirmation dialog; on confirm, services teardown, ports free
- [ ] Cmd+Q triggers orderly shutdown (no confirmation; honor power-user signal)
- [ ] `--uninstall` with launcher already dead: confirms orphans (`pty-server`, `webhook-tunnel`, `playground`) are swept
- [ ] Port 5199 collision: launcher shows osascript dialog "Port 5199 is in use" and exits

### Stack 2 — Wizard UX (Issues 1+2+3)

**Decisions covered:**
- [ ] #1  Per-service progress checklist via SSE
- [ ] #6  Extract progress: running count
- [ ] #15 Wait-healthy 60s soft / 90s hard / +60s extendable
- [ ] #16 "Open anyway" only when playground is healthy
- [ ] #19 SSE-connect 20s deadline
- [ ] #32 Service readiness probes target user-facing paths

**Files (new):**
- [ ] `apps/studio-installer/src-tauri/src/commands/wait_health.rs`

**Files (edit):**
- [ ] `apps/studio-installer/src-tauri/Cargo.toml` (eventsource-client)
- [ ] `apps/studio-installer/src-tauri/src/commands/mod.rs` + `lib.rs` (register)
- [ ] `apps/studio-installer/src/lib/installer.ts`
- [ ] `apps/studio-installer/src/lib/store.svelte.ts`
- [ ] `apps/studio-installer/src/steps/Launch.svelte`
- [ ] `apps/studio-installer/src/steps/Extract.svelte` (running count UI)
- [ ] `apps/studio-installer/src/steps/Download.svelte` (subtitle flip-order)
- [ ] `tools/friday-launcher/project.go` (probe-path update for Decision #32)
...
### Stack 3 — .app bundle + migration (Issue 5)

**Decisions covered:**
- [ ] #3  Bundle id `ai.hellofriday.studio-launcher`
- [ ] #7  `/Applications` only, prompt for admin if needed
- [ ] #8  v0.0.8 → v0.0.9 migration mandatory
- [ ] #11 No cutover compat for v0.1.15
- [ ] #12 Pre-flight: missing binaries → osascript dialog + Quit
- [ ] #14 LaunchAgent label keeps `ai.hellofriday.studio`
- [ ] #17 Service status state machine
- [ ] #20 Strip `com.apple.quarantine` xattr post-extract
- [ ] #23 xattr strip dispatches on cp privilege level
- [ ] #25 Supervised binaries live in `~/.friday/local/bin/`
- [ ] #26 License files bundled in `bin/`
- [ ] #27 License files come from pinned GitHub raw URLs
- [ ] #29 LaunchAgent autostart targets `open -b ai.hellofriday.studio-launcher`
- [ ] #30 Migration extracts to staging dirs + atomic swap
- [ ] #31 Pre-extract cleanup of stale staging + backup dirs
- [ ] #35 Build-time Info.plist assertion
- [ ] #36 Migration preserves the user's autostart preference
- [ ] #37 Migration writes `~/.friday/local/.installed` on success

**Files (new):**
- [ ] `tools/friday-launcher/preflight.go`
- [ ] `tools/friday-launcher/preflight_test.go`

**Files (edit / rewrite):**
- [ ] `scripts/build-studio.ts` — substantial rewrite: `.app` bundling, `bin/` subdir, license URL downloads, Info.plist assertion
- [ ] `.github/workflows/studio-build.yml` — codesign --deep, notarization
- [ ] `tools/friday-launcher/main.go` — `binDir` default → `~/.friday/local/bin/`, pre-flight call site
- [ ] `tools/friday-launcher/autostart_darwin.go` — `open -b` ProgramArguments
- [ ] `tools/friday-launcher/preflight_dialog_darwin.go` — extend with missing-binaries variant
- [ ] `tools/friday-launcher/preflight_dialog_windows.go` — extend
- [ ] `tools/friday-launcher/uninstall.go` — detached `.app` cleanup helper
- [ ] `apps/studio-installer/src-tauri/src/commands/extract.rs` — **REWRITE** (split-destination + staging + atomic swap + migration sequence + autostart preservation + `.installed` marker write)
- [ ] `apps/studio-installer/src-tauri/src/commands/install_dir.rs` — return tuple (app dir + binaries dir)
- [ ] `apps/studio-installer/src-tauri/src/commands/launch.rs` — **already has .app-bundle resolution; no changes**
- [ ] `apps/studio-installer/src-tauri/src/commands/installed_marker.rs` — **already exists; wired in from `extract.rs`**

**Live macOS QA (coordinate with user — REQUIRES v0.0.8-installed test machine):**
- [ ] `mdfind -name "Friday"` returns the new `.app`
- [ ] First `open -b "ai.hellofriday.studio-launcher"` is silent (no Gatekeeper prompt)
- [ ] Reboot fires autostart via `open -b`; tray appears
- [ ] LaunchAgent plist `ProgramArguments` is the bundle-ID variant
- [ ] Migration negative-path: extract failure leaves v0.0.8 install intact
- [ ] Crash-recovery negative test: `kill -9` mid-extract; rerun cleans staging
- [ ] Backup-orphan recovery: pre-seeded `~/.friday/local/backup-99999/` is removed
- [ ] Autostart-disabled preserved: pre-disabled with `--autostart disable`, migration leaves plist absent
- [ ] Idempotency: rerun installer on migrated machine; migration sequence skipped
- [ ] LSUIElement=1 .app indexed by Spotlight on macOS Sonoma+
- [ ] Pinned LICENSE URLs return 200 + non-empty for nats-server, cloudflared, gh
...
5. **`v0.0.8 → v0.0.9` migration runs in `extract.rs`**: detect
   ...
   `open -b ai.hellofriday.studio-launcher`, then `open -b
   ai.hellofriday.studio-launcher`.
...
37 numbered decisions (§Decisions); 14 risk callouts (§Risks);
...
- First-run UX is silent — no Gatekeeper / "downloaded from the
  internet" prompts on the user's first `open -b
  ai.hellofriday.studio-launcher`.
...
### Caller response-handling table (NEW in v11)

The installer's update flow + the `--uninstall` flow both POST
to this endpoint as the preferred shutdown path, with a SIGTERM
fallback. Without a precise spec, two implementers will pick
two different fallback policies. v11 fixes the response → action
mapping:

| HTTP outcome | Caller action |
|--------------|---------------|
| `connection refused` (port not bound; v0.0.8 launcher or already-dead launcher) | Fall through to SIGTERM-and-poll path immediately |
| EOF / connection reset before headers | Fall through to SIGTERM (launcher died mid-handshake) |
| `202 Accepted` | Poll `launcher.pid` for removal up to 35s. Success when pid file is gone OR `kill -0 <pid>` returns ESRCH |
| `409 Conflict` (shutdown already in progress, per Decision #10) | Same as 202 — poll `launcher.pid` for removal up to 35s. The launcher is already on a shutdown path; just wait for it |
| 4xx other (400, 404, etc.) | Log error + body, fall through to SIGTERM (endpoint exists but is misbehaving — don't trust it) |
| 5xx | Log error + body, fall through to SIGTERM (launcher hit an internal error mid-shutdown — SIGTERM is the safety net) |
| Read timeout for the POST itself (>5s to receive headers) | Cancel the request, fall through to SIGTERM (handler hung) |
| TLS / DNS errors | N/A — endpoint is loopback HTTP only, no TLS, no DNS |

The 5s POST-receive timeout is independent of the 35s
poll-for-pid-removal timeout. The POST should return headers
fast (it's just a `CompareAndSwap` + a goroutine spawn);
...
- `starting` → `healthy`: readiness probe (HTTP 200 against
  ...
  probe `<path>` MUST exercise the same handler stack a user-
  side consumer would hit — for playground today, `/` (the root
  is a public landing page). For services with auth-walled
  root paths, probe a different *publicly-reachable* path that
  still drives the real router (e.g. `/login`, a public landing,
  a public version endpoint) — NOT a sidecar `/health`
  (Decision #32). The principle is "probe exercises the real
  handler stack at a public entry point"; that's what makes
  "all healthy" actually mean "all usable".
...
// HealthCache reads from the global `shuttingDown` atomic that
// already exists in main.go. We pass it as a *atomic.Bool so the
// HTTP handlers and the existing tray-poll goroutine share one
// source of truth. (Per Decision #33, the existing two-atomic
// pattern stays — HealthCache adds nothing new.)
func handleShutdown(shuttingDown *atomic.Bool) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        // 409 contract: probe visibility flag. Doesn't gate the
        // shutdown itself — that's performShutdown's job via the
        // existing shutdownStarted CAS.
...
// performShutdown ALREADY EXISTS in main.go (line ~269). It uses
// the established two-atomic pattern: shutdownStarted gates the
// one-shot work (CAS), shuttingDown is the visibility flag the
// tray + (now) HTTP handler read. Decision #33 confirms this
// pattern handles concurrent triggers correctly — tray Quit +
// HTTP POST + NSApp will-terminate all funnel through here, and
// shutdownStarted ensures supervisor.Shutdown / sweep /
// srv.Shutdown each run exactly once.
//
// Existing main.go code (paraphrased — DO NOT re-implement):
//   func performShutdown(reason string) {
//       if !shutdownStarted.CompareAndSwap(false, true) { return }
//       shuttingDown.Store(true)
//       // ... supervisor.Shutdown(), SweepByBinaryPath, srv.Shutdown ...
//   }
//
// v15 wiring change for the new HTTP server: pass the global
// `&shuttingDown` into HealthCache so the handler's 409 probe
// reads the same atomic the tray reads, AND ensure
// performShutdown also calls srv.Shutdown(ctx) as its LAST step
// (after sweep, before releasePidLock). NO new CAS, NO new gate.
```
...
Order in `performShutdown` (Decision #33 makes this idempotent
across all trigger paths):
1. **CAS gate**: `if !cache.shuttingDown.CompareAndSwap(false,
   true) { return }` — concurrent callers from tray Quit, NSApp
   will-terminate, and HTTP POST all funnel through here; the
   first wins, the rest are no-ops.
2. Cache returns 503 from this point on; tray title flips to
   "Stopping…"
3. `supervisor.Shutdown()` (waits up to 30s)
4. `processkit.SweepByBinaryPath(binDir)` (catch orphans)
5. Close HTTP server with `srv.Shutdown(ctx)` and 2s timeout
6. `releasePidLock()` + `closeJob()`
...
spawns the new launcher via `open -b
ai.hellofriday.studio-launcher` (per Decision #29), which goes
through LaunchServices. Cold-cache LaunchServices spin-up +
...
- `apps/studio-installer/src-tauri/src/commands/extract.rs`
  (**rewritten end-to-end** — the existing `extract_archive`'s
  whole-install-dir `.bak`-rollback pattern (`dest_path.with_extension("bak")`)
  AND `terminate_studio_processes`'s SIGTERM-and-poll are
  removed and replaced by: split-destination iteration with
  manual `archive.entries()` for progress, per-component `.new`
  staging, `~/.friday/local/backup-<pid>` swap, HTTP-shutdown
  POST + SIGTERM fallback per the response table, migration
  sequence — see Issue 5 + §Migration. Channel parameter for
  per-entry progress events.)
...
**Already done (v15 alignment note):**
`apps/studio-installer/src-tauri/src/commands/launch.rs:32-54`
already resolves the launcher binary as
`Friday Studio.app/Contents/MacOS/friday-launcher` with a flat-
bundle fallback. The **consumer side** of the .app layout is
implemented; the **producer side** (build script emitting the
.app) is what's missing. Implementers working on Stack 3 should
NOT rewrite the `launch.rs` resolution — the existing logic
already handles both layouts and only needs the build-side
change to start producing .app bundles for the macOS-bundle code
path to fire.
...
5. **Strip quarantine xattr post-extract for silent first-launch.**
   ...
   `open -b ai.hellofriday.studio-launcher`, Gatekeeper would
   show "Friday Studio is from an identified developer—open?"
   or similar. We pre-empt the prompt by stripping the xattr.
...
7. **Pre-flight check via `osascript`: missing/corrupt binaries OR
   ...
   Both dialogs use the same osascript helper. Before showing the
   dialog, the launcher writes a diagnostic log to a reliable
   path so the user has something to share with support
   (Decision #34, **path updated in v13**). The log goes to
   `~/.friday/local/logs/launcher-startup.log` — co-located with
   the launcher's normal logs, predictable, easy for users to
   reference. We `mkdir -p` the dir first since a fully-broken
   install might lack it. If `mkdir` fails (extreme edge case —
   `~/.friday` not writable, full disk, etc.), fall back to
   ...
   func writeStartupErrorLog(reason string, details map[string]string) string {
       // Preferred: ~/.friday/local/logs/launcher-startup.log
       logDir := filepath.Join(homeDir(), ".friday", "local", "logs")
       logPath := filepath.Join(logDir, "launcher-startup.log")
       if err := os.MkdirAll(logDir, 0o755); err != nil {
           // Fallback: os.TempDir() so the log still lands somewhere
           logPath = filepath.Join(os.TempDir(), "friday-launcher-startup.log")
       }
       f, err := os.OpenFile(logPath,
           os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
       if err != nil {
           return "" // best-effort; dialog still shows
       }
       defer f.Close()
   ...
   func showMissingBinariesDialog(missing []string) string {
       ...
       exe, _ := os.Executable()
       logPath := writeStartupErrorLog("pre-flight", map[string]string{
           "missing_binaries": strings.Join(missing, ","),
           "bin_dir":          binDir,
           "exe":              exe,
   ...
   Log writes are best-effort — if `OpenFile` fails, the dialog
   still shows (just without the log path). The append-mode
   open lets repeated startup failures accumulate in the same
   file; an experienced user / support eng can read the whole
   history.
...
- `scripts/build-studio.ts` — substantial rewrite:
  ...
  - **Info.plist assertion (NEW in v13, Decision #35):** after
    generating the .app's `Info.plist`, parse it and assert
    `CFBundleIdentifier === "ai.hellofriday.studio-launcher"`
    AND `CFBundleExecutable === "friday-launcher"`. A typo in
    either field would silently produce a tarball where every
    user's `open -b` autostart fails — catch it at build time
    instead of in the field. Build fails loudly with the
    actual vs expected values on mismatch.
...
- `apps/studio-installer/src-tauri/src/commands/extract.rs`
  (**rewritten end-to-end** — see §Issue 1+2+3 file list for
  the full description. Migration sequence + Decision #31
  pre-extract cleanup + Decision #36 autostart preservation
  +  call to `installed_marker::write_installed("0.0.9")` at
  the end.)
- `apps/studio-installer/src-tauri/src/commands/installed_marker.rs`
  (**already exists**; migration calls `write_installed`. No
  new code here — just wired in from `extract.rs`. Decision
  #37.)
- `apps/studio-installer/src-tauri/src/commands/install_dir.rs`
  (return tuple: app dir + binaries dir)
- `apps/studio-installer/src-tauri/src/commands/launch.rs`
  (**already has .app-bundle resolution** at lines 32-54
  with flat fallback — no changes needed. Listed here for
  cross-reference so implementers don't rewrite it.)
...
5. **Replace installer's `terminate_studio_processes` with HTTP
   ...
   installer's update-flow shutdown becomes a single HTTP call
   followed by a poll for `launcher.pid` removal (up to 35s),
   instead of SIGTERM + pid-poll. Cleaner semantics; shares the
   same teardown code path as tray-Quit. The full response →
   action mapping lives in cross-cutting §Caller response-handling
   table — connection refused / EOF / 4xx / 5xx / timeout all
   fall through to SIGTERM-and-poll; only 202 and 409 wait the
   full 35s for pid removal.

6. **`--uninstall` runs `SweepByBinaryPath` unconditionally.** New
   flow:
   1. Try graceful shutdown via `POST /api/launcher-shutdown`
      (new HTTP endpoint). Apply the cross-cutting §Caller
      response-handling table: 202/409 → poll `launcher.pid` for
      removal up to 35s; everything else → fall through to
      SIGTERM. (The most common "everything else" case during
      `--uninstall` is connection-refused, because the launcher
      was already manually killed before `--uninstall` ran.)
   2. **Always** run `processkit.SweepByBinaryPath(binDir)` after
      step 1 — catches orphans whose parent (the launcher) was
      killed externally and never got a chance to clean up. Print
      `✓ swept N orphan processes` in the user-facing output if
      N > 0.
   3. **Then** disable autostart, remove state.json, remove pids/.
...
| LaunchAgent target       | `~/.friday/local/friday-launcher`         | `/usr/bin/open -b ai.hellofriday.studio-launcher --args …` |
...
3. **Capture prior autostart preference, then disable old
   autostart.** The LaunchAgent label (`ai.hellofriday.studio`)
   is unchanged across versions. Before unloading, **record
   whether the plist file existed** — that's the proxy for
   "did the user have autostart enabled before the upgrade?"
   On v0.0.8, `--autostart disable` removes the plist file
   entirely; absent file = user explicitly disabled (Decision
   #36):
   ```rust
   let plist_path = home.join("Library/LaunchAgents/ai.hellofriday.studio.plist");
   let user_had_autostart = plist_path.exists();
   ```
   Then unload + (let step 9 decide whether to overwrite):
   `launchctl unload ~/Library/LaunchAgents/ai.hellofriday.studio.plist`
   so any loaded plist (pointing at the old path) is unloaded.
   **Ignore non-zero exit** from `launchctl unload` — it returns
   1 if the plist isn't loaded, which is the expected state on
   fresh installs and on machines where the user manually
   unloaded it. Carry `user_had_autostart` through to step 9.
4. **Pre-extract cleanup of stale staging + backup dirs.**
   Before writing anything new, defensively remove any leftover
   staging or backup dirs from a prior crashed run (Decision #31):
   ```rust
   let _ = fs::remove_dir_all("/Applications/Friday Studio.app.new");
   let _ = fs::remove_dir_all(home.join(".friday/local/bin.new"));
   // v14: backup target lives under ~/.friday/local/ (per-user,
   // outside LaunchServices's index path). Sweep any leftovers
   // from a prior crashed migration.
   for entry in glob::glob(&home.join(".friday/local/backup-*").to_string_lossy())
   ...
   but is partially-written or stale, removing it is the only safe
   thing to do anyway. This makes step 5 idempotent under any
   prior failure mode (Rust error, OS kill, power loss).

   The `~/.friday/local/backup-*` cleanup covers the crash-
   between-step-6-and-step-7 case (a migration renamed the old
   `.app` aside but didn't get to remove the backup). Putting
   the backup under `~/.friday/local/` instead of `/Applications`
   (Decision #31 v13 update) means LaunchServices doesn't index
   it — `open -b` resolves only to the live bundle, never the
   orphan, even if the orphan survives a reboot. Per-user path
   (v14 update) avoids cross-user `/tmp` glob interference.
...
6. **Atomic swap.** Once extraction completes successfully:
   - Set backup path: `let bak = home.join(format!(".friday/local/backup-{}", std::process::id()));`
   - `mv /Applications/Friday Studio.app <bak>` (or skip if no
     prior `.app` exists). Out of `/Applications` so
     LaunchServices can't index the orphan.
   - `mv /Applications/Friday Studio.app.new /Applications/Friday Studio.app`
   - `mv ~/.friday/local/bin.new ~/.friday/local/bin`
   The `mv` of the live `.app` to `~/.friday/local/` is
   cross-filesystem on most setups (`/Applications` is on `/`,
   `~` may be on `/Users`'s own volume), so it's a copy-then-
   delete under the hood — slower than a same-FS rename, but
   still O(seconds) for our small bundle and atomic at the
   destination. The within-`/Applications` rename
   (`.new` → `.app`) IS same-FS and atomic; that's the swap
   that matters for live-path correctness. After this point
...
7. **Clean up old flat-layout binaries** from the v0.0.8 paths.
   ...
   - `rm -rf <bak>` (the `~/.friday/local/backup-<pid>` from
     step 6, if created)
...
9. **Conditionally re-register autostart via `open -b` (bundle
   ID).** Honor the `user_had_autostart` flag captured in step 3
   (Decision #36):
   ```rust
   if user_had_autostart {
       enable_autostart()?;  // writes the new plist
       log::info!("autostart re-registered with new layout");
   } else {
       log::info!("autostart was previously disabled by user; not re-enabling");
   }
   ```
   When re-registered, the new LaunchAgent's `ProgramArguments`
   is `["/usr/bin/open", "-b", "ai.hellofriday.studio-launcher",
   "--args", "--no-browser"]` — goes through LaunchServices,
   resolved by bundle ID so a stray copy of "Friday Studio.app"
   elsewhere on disk can't shadow it (see Decision #29 for why
   bundle-ID resolution is preferred over `-a`-by-name and over
   direct `Contents/MacOS/friday-launcher` exec).
10. **First launch.** Spawn the new launcher via `open -b
    "ai.hellofriday.studio-launcher"` instead of direct
    execution — uses LaunchServices, deterministic by bundle
    ID. With quarantine stripped, this is silent. Bonus:
    registers the .app with LaunchServices so Spotlight indexes
    it immediately rather than waiting for the periodic mds
    re-scan.
11. **Write the installed-version marker** (NEW in v15,
    Decision #37). The repo already has
    `apps/studio-installer/src-tauri/src/commands/installed_marker.rs`
    with a `write_installed(version: String)` function that
    atomically writes JSON to `~/.friday/local/.installed`.
    Migration calls it at the end:
    ```rust
    write_installed(env!("CARGO_PKG_VERSION").to_string())?;
    ```
    Two purposes:
    - **Future migrations dispatch on this marker.** v0.0.9 →
      v0.1.x will read `.installed.version` and dispatch
      directly — no more per-version Mach-O sniff
      heuristics.
    - **Idempotency signal.** A v0.1.16 installer run on an
      already-migrated machine reads the marker, sees
      `version == "0.0.9"` (or whatever's current), and skips
      the v0.0.8 → v0.0.9 migration sequence entirely. (The
      installer already does some idempotency — this codifies
      it for the migration too.)
    The v0.0.8 → v0.0.9 migration still detects v0.0.8 by
    Mach-O sniff because v0.0.8 predates `installed_marker.rs`
    and never wrote the file. From v0.0.9 onward the marker
    is the source of truth.

### Where the migration code lives

`apps/studio-installer/src-tauri/src/commands/extract.rs`. The
existing `extract_archive` function (with its whole-install-dir
`.bak` rollback via `dest_path.with_extension("bak")`) and
`terminate_studio_processes` (SIGTERM-and-poll) are **rewritten
end-to-end**, not extended — their structure is incompatible
with the split-destination + per-component staging + atomic-swap
+ HTTP-shutdown flow this plan introduces. v15 alignment note:
implementer should treat extract.rs as a fresh module, with the
existing functions deleted. The staging-then-swap helpers live
alongside; `installed_marker.rs::write_installed` is called at
the end of the migration to record the new version.

### Test before shipping

Take a v0.0.8-installed machine (lcf has one), run the v0.1.16
installer through Update mode end-to-end, verify:
- LaunchAgent plist `ProgramArguments` is `/usr/bin/open -b ai.hellofriday.studio-launcher --args --no-browser`
- `~/.friday/local/friday-launcher` is gone
- `~/.friday/local/Friday Studio.app/` is gone (if it existed)
- `/Applications/Friday Studio.app` is present, no `.new` sibling
- `~/.friday/local/backup-*` is empty (migration step 7 cleaned
  up the backup; nothing leftover under LaunchServices-invisible
  path either)
- `mdfind kMDItemCFBundleIdentifier == 'ai.hellofriday.studio-launcher'`
  returns exactly ONE path (the live .app), not two
...
- First `open -b "ai.hellofriday.studio-launcher"` is silent
  (no Gatekeeper prompt)
- Quit + relaunch via Spotlight works (Spotlight uses
  LaunchServices internally; same resolution path)
- Reboot: autostart fires the .app launcher via `open -b`
  resolving the bundle ID, not the old binary path
...
- **Negative test (backup-orphan recovery):** simulate a crash
  between migration step 6 (rename live `.app` to
  `~/.friday/local/backup-<pid>`) and step 7 (remove backup) by
  manually creating `~/.friday/local/backup-99999/` (a copy of
  an old `.app` bundle with the same `CFBundleIdentifier`)
  before running the installer. After install, verify the
  backup is gone, AND the pre-existing copy didn't shadow the
  live one mid-install (`mdfind kMDItemCFBundleIdentifier ==
  'ai.hellofriday.studio-launcher'` returns exactly one path —
  the live `/Applications/Friday Studio.app`). Exercises
  Decision #31 (v14 update).
- **Negative test (autostart-disabled preserved across upgrade,
  Decision #36):** on the v0.0.8-installed test machine, run
  `friday-launcher --autostart disable` first (removes the
  plist), then run the v0.1.16 installer through the migration
  end-to-end. After install, verify
  `~/Library/LaunchAgents/ai.hellofriday.studio.plist` is
  STILL absent (migration respected the user's disabled
  preference; step 9 was skipped). Reboot: launcher does NOT
  auto-start at login. User can re-enable by running
  `friday-launcher --autostart enable` from the live binary
  (manually launching it from Spotlight first).
- **Positive test (autostart-enabled preserved across upgrade,
  Decision #36):** on the v0.0.8-installed test machine, leave
  autostart enabled (default), run the v0.1.16 installer.
  After install, verify the new plist is present at the same
  path with the new `open -b` ProgramArguments. Reboot: tray
  appears at login.
- **Negative test (legacy v12-era `.app.bak` in `/Applications`):**
  if the user upgraded a v12-installed machine through to v14,
  the prior install may have left `/Applications/Friday Studio.app.bak`
  from a v12-era crash. Manually create that path before running
  the v14 installer. v14's pre-extract cleanup doesn't sweep
  this path (Decision #31 v14 only sweeps
  `~/.friday/local/backup-*`). Verify the orphan is invisible to
  `open -b` resolution AFTER the migration completes (the live
  .app is in `/Applications`; LaunchServices may still index the
  orphan but the live one wins by being the most-recent +
  correctly-signed). Acceptable: legacy v12 fleet is empty
  (~1 user, never shipped to fleet). Test exists to confirm
  `open -b` doesn't degrade in this edge case.
...
- **LaunchAgent target**: `/usr/bin/open -b
  ai.hellofriday.studio-launcher --args --no-browser` (NOT
  `-a "Friday Studio"` and NOT a direct
  `Contents/MacOS/friday-launcher` path) so autostart resolves
  via LaunchServices by bundle ID — deterministic, won't pick
  a stale copy elsewhere on disk. **Limitation:** `--args`
  only delivers when the .app isn't already running; against a
  live launcher the args are dropped silently. Don't try to
  pass meaningful runtime args to a running launcher via
  `open` — route through HTTP on port 5199 instead.
...
- **Bundled LICENSEs**: `bin/{nats-server,cloudflared,gh}-license`
  ...
  also gets one).
- **Service readiness probes exercise the real handler stack at
  a publicly-reachable path**: a service is reported `healthy`
  once its readiness probe returns 200. Probe path must drive
  the production router (e.g. `/` for playground today, a public
  landing or login page for an auth-walled service in the
  future) — NEVER a sidecar `/health` that bypasses the real
  handler. This is what makes "all healthy" actually mean "all
  usable". Future supervised services follow this pattern in
  `project.go`.
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v15.md`
  § Migration.
...
5. **Test order:** ship v0.1.16 installer first; manually
   ...
   re-registers autostart via `open -b` (bundle ID) → launcher exposes
   ...
   the .app → silent first `open -b` → Quit tears it all down,
...
- **`open -b` autostart vs direct exec**: routing autostart
  through LaunchServices means launchd's child is `open`, not
  the launcher itself. The launcher detaches anyway (setsid on
  Unix), so process-tree semantics are unchanged. But if
  `open -b` itself fails (LaunchServices DB corruption, or the
  bundle ID isn't registered yet because no `open` has happened
  since the .app was placed) the launcher never starts and
  there's no error to a user-facing UI. Mitigated: migration
  step 10 runs `open -b` once post-install, which registers the
  bundle ID with LaunchServices. Subsequent autostart firings
  reuse the cached registration. If LaunchServices DB is
  corrupted post-install, "tray never appears at login" is the
  symptom; user re-runs installer to re-register. Acceptable.
- **`open` second-launch arg drop**: per Decision #29, `open -b
  ai.hellofriday.studio-launcher --args …` only delivers args
  on a fresh launch. Args sent to an already-running .app are
  silently dropped. Today this only matters for autostart (one
  call site, fresh launch by definition), but future code that
  tries to pass meaningful args via this channel will hit the
  bug. Marked in CLAUDE.md as a foot-gun; routing future-runtime
  args through the HTTP server on port 5199 is the supported
  path.
...
with staging+swap, autostart via `open -b` (bundle ID), license downloads
...
ProgramArguments switched to `open -b` with the bundle ID),
...
| 3 | `tools/friday-launcher/autostart_darwin_test.go` (new; **updated in v12**) | Plist written by `enableAutostart()` has `ProgramArguments` of length 5: `["/usr/bin/open", "-b", "ai.hellofriday.studio-launcher", "--args", "--no-browser"]` — bundle ID parameterized via const, must match Decision #3. Verify plist parses cleanly and `currentAutostartPath()` returns `/usr/bin/open` (not the launcher binary). Negative test: assert the plist does NOT contain `"-a"` or `"Friday Studio"` (display-name resolution would re-introduce stale-.app shadowing). |
...
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend) | Migration: pre-existing `~/.friday/local/friday-launcher` is removed; pre-existing `~/.friday/local/Friday Studio.app/` is removed; LaunchAgent plist is unloaded then overwritten to `open -b ai.hellofriday.studio-launcher` invocation; `xattr` shows no `com.apple.quarantine` after extract (verify via `xattr -p`). |
...
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend; NEW in v10, extended v11, path updated v13, **per-user path v14**) | Pre-extract cleanup of stale staging + backup dirs (Decision #31): seed `~/.friday/local/bin.new/`, `/Applications/Friday Studio.app.new`, AND `~/.friday/local/backup-12345/` with partial-content fixtures that simulate prior crashed runs; invoke extract; assert all three stale paths are removed at step 4 before the new extraction writes anything. Glob-pattern sweep of `~/.friday/local/backup-*` covers any pid suffix. End state matches a clean install with no stale files. |
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend; **NEW in v14**) | Decision #36 — autostart preference preservation. Two scenarios: (a) plist file present pre-migration → migration completes with new plist present (autostart re-registered); (b) plist file absent pre-migration → migration completes WITHOUT writing a new plist (user's `--autostart disable` choice respected). Mock the home directory; pre-seed plist or omit; run migration; assert post-state matches the corresponding scenario. Critical to assert step 9 doesn't run `enable_autostart()` when `user_had_autostart` is false. |
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` + `installed_marker.rs` (extend; **NEW in v15**) | Decision #37 — `.installed` marker. After successful migration assert `~/.friday/local/.installed` exists with `version == "0.0.9"`. Idempotency test: pre-seed the marker with `version: "0.0.9"`, run the installer; assert the migration sequence (Mach-O sniff + autostart unload + extract + swap) is SKIPPED — installer recognizes it's already at target version and exits early. |
| 1 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (or wherever the shutdown POST helper lives; **NEW in v11**) | Caller response-handling table (cross-cutting §): mock HTTP server that returns each row's response (connection-refused via `tcp_listener.drop()`, 202, 409, 400, 500, header-read timeout); assert the helper's resulting action matches the table (202/409 → poll-for-pid-removal, everything else → fall through to SIGTERM path). |
| 1 | `tools/friday-launcher/main_test.go` or `healthsvc_test.go` (new; **NEW in v12**) | Concurrent-shutdown CAS gate (Decision #33): spawn N goroutines that each call `performShutdown(reason)` simultaneously; assert the inner shutdown work (`supervisor.Shutdown`, `SweepByBinaryPath`, `srv.Shutdown`) runs exactly once; assert N-1 callers return early via the early-return log line. Use a fake supervisor + counters to verify each step ran exactly once across the concurrent invocations. |
| 3 | `tools/friday-launcher/preflight_dialog_darwin_test.go` (new; NEW in v12, **path updated v13**) | Startup-error log path (Decision #34): override HOME to a tempdir; call `showMissingBinariesDialog(["foo", "bar"])` with osascript stubbed to return "Quit"; assert `<HOME>/.friday/local/logs/launcher-startup.log` exists, contains `"missing_binaries: foo,bar"`, contains the exe path + OS arch, and the dialog body string returned to the caller embeds the log path. Same for `showPortInUseDialog`. Append-mode write: invoke twice, assert both entries are in the file. **Fallback test:** make `<HOME>/.friday/local/logs/` un-creatable (e.g. `chmod -w <HOME>/.friday/local`), invoke; assert log lands at `os.TempDir()/friday-launcher-startup.log` instead. |
...
| 3 | `scripts/build-studio.ts` (build-time assertion; **NEW in v13**) | Decision #35 — after writing `Info.plist`, parse and assert `CFBundleIdentifier === "ai.hellofriday.studio-launcher"` AND `CFBundleExecutable === "friday-launcher"`. Build fails on mismatch with actual-vs-expected. Negative test: tweak the plist template to a wrong ID, run the build, assert the build exits non-zero with a message naming the actual + expected ID. |
...
- Live LaunchServices integration of `open -b` autostart — manual
  verification on test hardware (CI's macOS runners don't have
  user-session LaunchServices state).

## Decisions (confirmed 2026-04-27, v15 pass)
...
29. **LaunchAgent autostart targets `open -b
    "ai.hellofriday.studio-launcher"`, not the bundle name and
    not `Contents/MacOS/friday-launcher` directly** (NEW in v9;
    limitation documented in v10; **bundle-ID resolution in
    v12**) — going through LaunchServices means Info.plist
    (LSUIElement, env, icon, quarantine flags) is consulted on
    every boot, matching the user-launched path via Spotlight.
    Direct exec of the bundle's binary skips LaunchServices,
    which would create subtle dev/prod parity bugs (e.g. an
    LSUIElement update only takes effect after the user re-
    launches via Finder).

    **v12 — use `-b` (bundle ID), not `-a` (display name):**
    name-based resolution can pick a stale .app on disk (e.g.
    an old copy in `~/Downloads`, or a previous install in
    `~/Applications`). Bundle-ID resolution is deterministic.
    Decision #3 already commits to the bundle ID
    `ai.hellofriday.studio-launcher` for code signing — using
    it for `open` is just consistency. The plist's
    `ProgramArguments` becomes:

    ```xml
    <array>
      <string>/usr/bin/open</string>
      <string>-b</string>
      <string>ai.hellofriday.studio-launcher</string>
      <string>--args</string>
      <string>--no-browser</string>
    </array>
    ```

    Trade-off: launchd's child is `open`, not the launcher
    itself, but the launcher detaches anyway so process-tree
    semantics are unchanged.

    **v10 limitation note (still applies under `-b`):** `open
    -b … --args --no-browser` only delivers `--no-browser` to
    a *fresh* launch. If the .app is already running, `open`
    brings the existing instance to the front via an Apple
    Event and the `--args` are dropped silently. This is
    acceptable because the only call site is autostart at
    login, where the launcher is by definition not yet running.
    Future code MUST NOT pass meaningful args via `open -b
    ai.hellofriday.studio-launcher --args …` against an
    already-running launcher — they will not arrive. If a use
    case ever needs to deliver runtime args to an already-
    running launcher, route through the existing HTTP surface
    on port 5199 instead. Captured in CLAUDE.md so future
    agents don't re-discover the limitation the hard way.
...
31. **Pre-extract cleanup of stale staging + backup dirs** (NEW
    in v10, **extended in v11**) — Decision #30's staging-then-
    swap pattern is robust against Rust error paths but doesn't
    cover `kill -9` / power loss / OS crash mid-extract. Without
    cleanup, a second installer run could either collide on the
    ...
    `remove_dir_all` returns NotFound, which is fine.

    **v11 extension:** the cleanup also wiped
    `/Applications/Friday Studio.app.bak`. Migration step 6
    creates a `.bak` (renaming the old `.app` aside before
    swapping the new one in); step 7 normally removes it.
    A crash between step 6 and step 7 leaves the `.bak`
    orphan on disk.

    **v13 update — backup lives outside `/Applications`:**
    v11/v12 put the `.bak` *inside* `/Applications`, alongside
    the live `.app`. Both bundles advertise the same
    `CFBundleIdentifier` (`ai.hellofriday.studio-launcher`).
    LaunchServices indexes everything in `/Applications`, and
    its resolution between same-ID bundles is non-deterministic —
    `open -b` after a step-6/step-7 crash could resolve to the
    orphan `.bak` (with the OLD launcher binary) for an entire
    reboot cycle. v13 moved the backup to `/tmp` to keep it out
    of LaunchServices's index.

    **v14 update — backup target is per-user:** v13's `/tmp`
    location is shared across users on macOS. A multi-user
    machine could see one user's pre-extract glob sweep wipe
    another user's backup. v14 moves the target to
    `~/.friday/local/backup-<pid>` instead — per-user by
    definition, ALSO outside LaunchServices's index path,
    matches the existing install-paths convention. Pre-extract
    cleanup updated accordingly:
    ```rust
    let _ = fs::remove_dir_all("/Applications/Friday Studio.app.new");
    let _ = fs::remove_dir_all(home.join(".friday/local/bin.new"));
    for entry in glob::glob(&home.join(".friday/local/backup-*").to_string_lossy())
    ...
    ```
    Combined with Decision #30's atomic swap, this makes
    migration fully crash-recoverable: the only states
    observable on disk after any failure are "v0.0.8 layout
    intact" or "v0.0.9 layout live" — never a half-written
    hybrid, never a LaunchServices-indexable orphan, never a
    cross-user collision.
32. **Service readiness probes target user-facing paths, not
    sidecar `/health` endpoints** (NEW in v10, **caveat
    tightened in v11**) — the wizard enables "Open in Browser"
    when every supervised service reports `healthy` (Decision
    #1's per-service checklist). A service goes
    `starting → healthy` when its readiness probe returns 200.

    **The principle:** probe a *publicly-reachable* path that
    exercises the same handler stack as the user-loaded surface.
    The goal is to catch "process is up but the real handler
    isn't bound yet" races — which a sidecar `/health` does NOT
    catch. The probe needs to actually drive the SvelteKit
    router / API router / whatever ships the user-facing
    response, just at a public entry point.

    Concretely in `project.go`:
    - `playground` probes `http://127.0.0.1:5200/` (root path is
      a public landing page today)
    ...
    **v11 caveat — auth-walled root paths:** if a future service
    has authentication on `/` (e.g. `/` redirects to `/login`
    with a 302, or returns 401), `/` would never return 200 and
    the service would never go `healthy`. The right move is NOT
    to add a sidecar `/health` (that re-introduces the v10 bug).
    Instead: probe a *different publicly-reachable path that
    still exercises the real handler stack* — e.g. `/login`
    itself, an unauthenticated landing page, or a public version
    endpoint served by the same router. The contract is "probe
    exercises the production routing stack"; the path can be
    anything publicly reachable that satisfies that contract.
...
33. **`performShutdown` is the single CAS-gated choke point for
    all shutdown triggers** (NEW in v12, **aligned with existing
    code in v15**) — concurrent triggers (tray Quit, NSApp
    will-terminate, HTTP POST, `--uninstall`'s in-process call)
    must funnel through one CAS so `supervisor.Shutdown`,
    `SweepByBinaryPath`, and `srv.Shutdown(ctx)` each run
    exactly once.

    **v15 alignment with existing code:** prior plan revisions
    described this as a new piece to write. In fact,
    `tools/friday-launcher/main.go` (lines 50–56, 269–299)
    ALREADY implements this pattern with a two-atomic split:
    - `shutdownStarted atomic.Bool` — the one-shot CAS gate.
      `performShutdown` opens with `if
      !shutdownStarted.CompareAndSwap(false, true) { return }`.
    - `shuttingDown atomic.Bool` — the visibility flag, set
      inside `performShutdown` via `shuttingDown.Store(true)`,
      read by the tray-poll goroutine to render "Shutting
      down…".

    Decision #33 in v15 is therefore: **don't add anything
    new**. The HTTP handler reuses the existing `shuttingDown`
    atomic for its 409 probe (passed into `HealthCache` as a
    `*atomic.Bool`). The HTTP shutdown path calls the existing
    `performShutdown("http:shutdown")`; the existing
    `shutdownStarted` CAS already guarantees the work runs
    once across all triggers. The only NEW bit is wiring
    `srv.Shutdown(ctx)` into `performShutdown`'s sequence as
    the LAST step (after sweep, before `releasePidLock`).

    Failure mode v15 prevents: an implementer reading earlier
    plan revisions might add a separate
    `cache.shuttingDown.CompareAndSwap` in `handleShutdown` AND
    keep the existing `shutdownStarted.CompareAndSwap` in
    `performShutdown`. Two CAS gates = one redundant + a
    confusing readers' question of "which one wins?". v15
    closes that ambiguity: ONE gate, in `performShutdown`,
    using the EXISTING `shutdownStarted` atomic.
34. **Pre-flight + bind-failure errors write a diagnostic log
    to `~/.friday/local/logs/launcher-startup.log`** (NEW in v12,
    **path updated in v13**) — when the osascript dialog fires
    for missing binaries or port-in-use, the user's only
    artefact today is the dialog body. They click Quit and have
    nothing to share with support beyond "Friday Studio wouldn't
    start".

    v12 wrote to `os.TempDir()` (always-writable, but opaque on
    macOS where it resolves to `/var/folders/<hash>/T/...`).
    v13 writes to `~/.friday/local/logs/launcher-startup.log`
    instead — co-located with the launcher's normal logs,
    predictable path the user already knows, easier to reference
    in a bug report. `mkdir -p` ensures the dir exists; if mkdir
    fails (a fully-broken install with a non-writable
    `~/.friday`), fall back to
    `os.TempDir() + "/friday-launcher-startup.log"` so the log
    still lands somewhere.

    One entry per failed startup attempt (open in append mode so
    repeated failures accumulate). Each entry includes
    timestamp, error reason, bin_dir, exe path, OS+arch. The
    dialog body gains a `"Diagnostic log: <path>"` line so the
    user can copy and attach it. Best-effort: if both the
    primary and fallback paths fail (e.g. read-only
    filesystem), the dialog still shows without the log path.
    Same path is used by both pre-flight (missing binaries) and
    bind-failure (port-in-use) variants.
35. **Build-time assertion: Info.plist matches Decision #3 +
    Decision #29's expectations** (NEW in v13) —
    `open -b "ai.hellofriday.studio-launcher"` only works if
    the .app's Info.plist actually advertises that bundle ID.
    Code-signing (per Decision #3) also depends on the bundle
    ID matching the certificate. A typo in
    `scripts/build-studio.ts`'s plist template would silently
    ship a tarball where every user's autostart fails AND
    code-signing is invalid. v13 parses the generated
    Info.plist post-build and asserts:

    ```ts
    const plist = parsePlist(readFileSync(`${appDir}/Contents/Info.plist`));
    if (plist.CFBundleIdentifier !== "ai.hellofriday.studio-launcher") {
      throw new Error(`Info.plist CFBundleIdentifier mismatch: got ${plist.CFBundleIdentifier}, want ai.hellofriday.studio-launcher`);
    }
    if (plist.CFBundleExecutable !== "friday-launcher") {
      throw new Error(`Info.plist CFBundleExecutable mismatch: got ${plist.CFBundleExecutable}, want friday-launcher`);
    }
    ```

    Build fails loudly with the actual-vs-expected values on
    mismatch. Cheap insurance against a class of bugs that
    would otherwise only surface in production. Same constants
    as the launcherBundleID const used by `enableAutostart()`
    so a single source-of-truth change rebuilds + re-asserts.
36. **Migration preserves the user's autostart preference**
    (NEW in v14) — v13's migration step 9 unconditionally
    re-registered autostart via `enableAutostart()`. If the
    user had explicitly disabled autostart on v0.0.8 (via
    `friday-launcher --autostart disable`, which removes the
    plist file), the v0.0.9 migration silently re-enabled it
    — overriding the user's choice. v14 captures the prior
    plist's existence as a proxy for the user's preference at
    step 3:
    ```rust
    let plist_path = home.join("Library/LaunchAgents/ai.hellofriday.studio.plist");
    let user_had_autostart = plist_path.exists();
    ```
    Step 9 then conditionally re-registers:
    ```rust
    if user_had_autostart {
        enable_autostart()?;
    } else {
    ...
    ```
    Why "plist file existence" as the proxy: on v0.0.8,
    `--autostart enable` writes the plist; `--autostart disable`
    removes it. The file's presence is the authoritative signal.
    On the rare edge case where a user has the plist file
    present but unloaded via `launchctl unload` (without
    `--autostart disable`), we treat it as enabled — that
    matches what would happen on next reboot anyway (RunAtLoad
    fires it). Conservative interpretation: only the
    plist-file-absent case skips re-registration.
37. **Migration writes `~/.friday/local/.installed` on success**
    (NEW in v15) — the post-cleanup repo already has
    `apps/studio-installer/src-tauri/src/commands/installed_marker.rs`
    with `write_installed(version: String)` and
    `read_installed()` functions that atomically read/write a
    JSON marker `{ version, installed_at }` at
    `~/.friday/local/.installed`. The v0.0.8 → v0.0.9
    migration calls `write_installed("0.0.9")` as its final
    step (new step 11 in §Migration). Two purposes:

    - **Idempotency.** A v0.1.16 installer re-run on an
      already-migrated machine reads the marker and skips
      the migration sequence entirely — saves the user a
      30s migration if they accidentally double-run the
      installer.
    - **Future-migration dispatch.** v0.0.9 → v0.1.x will
      read `.installed.version` and dispatch directly. No
      more per-version Mach-O sniff; no need for the
      heuristic "is the launcher binary at the old path"
      check that v0.0.8 → v0.0.9 still uses (because v0.0.8
      predates the marker).

    Why this didn't add a new file: the marker
    infrastructure already exists in the codebase — v15 just
    wires the migration sequence to call it. (In v13 review
    a similar idea was rejected on "no backwards compat
    needed" grounds; v15 acknowledges the underlying file
    already shipped with the cleanup, so we get the benefit
    for free as long as the migration writes it.)
