<!-- v3 - 2026-04-23 - Generated via /improving-plans from docs/plans/2026-04-23-studio-installer-design.v2.md -->
...
- **Hides:** Direct `std::process::Command` spawning of all five processes in order (link → atlas → pty-server → webhook-tunnel → agent-playground) from `install_dir`, detached from the installer process. After spawning each process, calls `child.try_wait()` after 100ms to catch immediate exits (binary not found, permission denied, immediate crash) — returns `Err("Failed to start {name}: exited with code {N}")` if any process exits. Then TCP-polls port 5200 for up to 30s as the overall liveness gate. `tauri-plugin-opener` call to open browser.
- **Trust contract:** On `Ok`, all five processes passed the immediate-exit check and port 5200 became reachable within 30s, and the browser has been opened to `http://localhost:5200`. On `Err`, either a specific process failed to stay alive (message names the culprit and exit code) or port 5200 never became reachable within the timeout. Note: uses `std::process::Command` directly (not `tauri-plugin-shell`) — no capability allowlist needed since this runs in Rust, not JS.
...
- **`launch.rs`**: process that exits within 100ms → `Err` naming the process and exit code; all five processes stay alive and port 5200 opens → `Ok`; port 5200 never opens within 30s → `Err` with timeout message.
