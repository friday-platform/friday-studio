<!-- v4 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v3.md -->

# Installer + Launcher UX Fixes (v4)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation pending
**Supersedes:** v3 (`2026-04-27-installer-launcher-ux-fixes.v3.md`)
...
to ask the launcher anything except "did the pid file appear". v4 adds
...
- `tools/friday-launcher/healthsvc.go` (new — chi router, handlers,
  RWMutex cache, SSE channel, state machine)
...
| 90s (or 150s if extended) | Hard fail. Failed rows turn red. Two buttons:<br/>• **View logs** (always shown)<br/>• **Open anyway** (shown ONLY if playground service is healthy — see partial-success rule below) |
...
- **`wait_for_services` Tauri command** is a thin SSE-relay in
  ...
  - Two timeouts: 60s "soft" (UI swaps to long-wait copy + extend
    button), 90s "hard" (default end), extendable to 150s. If the
    launcher hasn't reported all services healthy at the hard
    deadline, emit `HealthEvent::Timeout { stuck: [name],
    playground_healthy: bool }` so the wizard can decide whether
    to show "Open anyway".
...
- `apps/studio-installer/src-tauri/src/commands/wait_health.rs` (new —
  SSE relay; staged 60s/90s/150s deadlines)
...
6. **Pre-flight check via `osascript`: missing/corrupt binaries →
   ...
   Sub-50 LOC; defense in depth — the wizard's pre-install check
   prevents the broken-binaries case for fresh installs, but
   pre-flight catches users who manually delete `~/.friday/local`.
...
- `tools/friday-launcher/main.go` (binDir default for .app context;
  pre-flight check call before systray.Run)
...
5. **Test order:** ship v0.0.9 platform first (existing v0.1.15
   ...
   with staged 60s/90s/150s deadline → tray bucket reads
   health-cache → Spotlight finds the .app → Quit tears it all
   down, `--uninstall` sweeps every orphan.
...
- **Wizard's 90s deadline + extension button**: a user on an
  *extremely* slow disk could need >150s. After the extension
  ...
  playground itself is the slow service, the user is stuck — we
  show "View logs" + a "Wait again" link that re-arms the
  extension. UX rarely-used safety net.

## Decisions (confirmed 2026-04-27, v4 pass)
...
12. **Pre-flight check: missing binaries → osascript dialog +
    ...
    NSAlert because by then NSApp is up. Different code paths;
    each tool fits its phase.
...
15. **Wait-healthy staged deadline: 60s soft / 90s hard /
    +60s extendable** — at 60s, swap the spinner copy to "this is
    taking longer than usual" and show a "Wait 60s more" button.
    Hard fail at 90s (or 150s if extended) with View Logs +
    optional Open Anyway.
16. **"Open anyway" only when playground is healthy** —
    ...
    stuck. If playground itself is unhealthy at the deadline,
    only "View logs" + "Wait again" are shown.
...
18. **HTTP server lifecycle decoupled from `RestartAll`** — the
    ...
    and only closes once in `performShutdown` (LAST step). SSE
