<!-- v6 - 2026-04-24 - Generated via /improving-plans from docs/plans/2026-04-23-studio-artifact-design.v5.md -->
...
`ReadHeaderTimeout: 10s` defends against slow-loris attacks on the request
side. No `ReadTimeout` or `IdleTimeout` — matching the cortex pattern exactly.
`WriteTimeout: 0` is required; do not use `pkg/server`'s `Listen()` for the
download port, as it defaults `WriteTimeout: 0` to 30 seconds.
...
apps/studio-artifact/
  main.go              — env load → config parse → service.New → metrics → http.Server (WriteTimeout:0, ReadHeaderTimeout:10s) → signal wait → graceful shutdown (30s)
  service/
