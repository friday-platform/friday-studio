<!-- v4 - 2026-04-23 - Generated via /improving-plans from docs/plans/2026-04-23-studio-installer-design.v3.md -->
...
- **Hides:** Platform-appropriate script format (`.sh` with `nohup` on macOS/Linux, `.bat` with `start /B` on Windows), PID file location (`~/.friday/local/pids/`), binary path construction from install dir, proper quoting of paths containing spaces (e.g. `"Friday Studio"` on Windows), platform env var injection, Node.js JWT generation for `ATLAS_KEY`.
- **Trust contract:** Script at returned path, when executed, starts all five Studio processes and is idempotent. The generated script sets **two categories** of environment:
  1. **User API keys** — sourced from `~/.friday/local/.env` (ANTHROPIC_API_KEY, OPENAI_API_KEY and any other vars present in that file).
  2. **Platform-internal env vars** — hardcoded in the script body, not expected in `.env`:
     - `ATLAS_LOCAL_ONLY=true` — skips remote credential fetching in the `link` service (single-user local mode)
     - `LINK_DEV_MODE=true` — enables Deno KV fallback in `link`
     - `ATLAS_KEY` — a local JWT generated inline via Node.js: `{alg:"HS256",typ:"JWT"}` header, payload with `sub:"local-user"`, signature `"local"` — accepted by `link` in `ATLAS_LOCAL_ONLY` mode to provide user identity for skill publish and workspace creation. See `docker/run-platform.sh` for the reference implementation.
     - `ATLASD_URL=http://localhost:8080` — webhook-tunnel's connection point to the daemon
     - `VITE_EXTERNAL_DAEMON_URL=http://localhost:8080` — agent-playground daemon proxy
     - `VITE_EXTERNAL_TUNNEL_URL=http://localhost:9090` — agent-playground tunnel proxy
     - Binary path overrides (`ATLAS_NPX_PATH`, `ATLAS_NODE_PATH`, `ATLAS_CLAUDE_PATH`, `ATLAS_SQLITE3_PATH`) derived from `install_dir`

  All paths in the script are quoted to handle the `Friday Studio` space in the install directory name.

**`src-tauri/src/commands/launch.rs`**
- **Interface:** `launch_studio(install_dir: String)` → `Result<(), String>`
- **Hides:** Direct `std::process::Command` spawning of all five processes in two ordered phases using `install_dir`. After spawning each process, calls `child.try_wait()` after 100ms to catch immediate exits. Conducts backend health polling before starting frontend processes. Finally polls port 5200 as overall liveness gate. `tauri-plugin-opener` call to open browser.
- **Startup sequence:**
  - **Phase 1 — backends:** spawn `atlas` (daemon, port 8080) → `try_wait(100ms)` → spawn `link` (port 3100) → `try_wait(100ms)` → poll `http://localhost:8080/health` up to 30s → poll `http://localhost:3100/health` up to 30s.
  - **Phase 2 — frontends:** (only after both backend health checks pass) spawn `agent-playground` (port 5200) → `try_wait(100ms)` → spawn `pty-server` (port 7681) → `try_wait(100ms)` → spawn `webhook-tunnel` (port 9090) → `try_wait(100ms)`.
  - **Final gate:** TCP-poll port 5200 up to 30s.
  - Open browser via `tauri-plugin-opener`.
- **Trust contract:** On `Ok`, all backend services (atlas, link) passed their health checks before any frontend process was spawned; all five processes passed the 100ms immediate-exit check; port 5200 became reachable within 30s; and the browser has been opened to `http://localhost:5200`. On `Err`, the message identifies whether the failure was a health check timeout (naming the service), an immediate-exit crash (naming the process and exit code), or port 5200 timeout. Uses `std::process::Command` directly (not `tauri-plugin-shell`) — no capability allowlist needed.
...
- **Hides:** `fetch_manifest` invocation (with 10s timeout error handling), `read_installed` call (None → fresh), semver comparison (`installed >= available` → current, `installed < available` → update), `check_running_processes()` call, step-sequence branching (fresh / update / current / running), Tauri command invocation sequence, `write_installed` call on successful extract, `delete_partial` call on checksum failure before retry, `check_download_complete` call to skip re-download on reopen, `mark_download_complete` call after successful `Done` event.
...
- **Checksum failure handling:** if `verify_sha256` returns `false`, `installer.ts` calls `delete_partial(platform)` before re-invoking `download_file`. `delete_partial` removes both the `.partial` and `.complete` files, fully resetting download state. This prevents resume from appending to a corrupt partial and prevents the checkpoint from skipping a required re-download.
...
PIDs written to `~/.friday/local/pids/`. Script polls `http://localhost:5200` (TCP connect, up to 30s) before opening browser. All paths in the script are quoted to handle the `Friday Studio` space in the install directory name.

The script sets platform-internal env vars inline (see `startup.rs` trust contract above) and sources `~/.friday/local/.env` for user API keys. See `docker/run-platform.sh` as the reference implementation for the full environment setup.
...
- The `ATLAS_KEY` JWT used in local mode is not cryptographically signed — it uses the literal string `"local"` as the signature component. The `link` service accepts this in `ATLAS_LOCAL_ONLY=true` mode. This is intentional: the single-user local install has no remote auth to validate against.
