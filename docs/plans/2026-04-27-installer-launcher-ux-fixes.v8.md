<!-- v8 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v7.md -->

# Installer + Launcher UX Fixes (v8)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v7 (`2026-04-27-installer-launcher-ux-fixes.v7.md`)

> **What v8 changes from v7 (real design changes, not polish):**
> two new requirements landed:
> 1. **Supervised binaries move to `~/.friday/local/bin/`** —
>    cleaner install layout, isolates binaries from state files
>    (pids/, logs/, state.json, .env). Cascades into binDir
>    resolution, sweep target, pre-flight paths, migration step,
>    and tarball staging.
> 2. **License files for third-party + our own binaries** —
>    Apache 2.0 / MIT compliance for nats-server, cloudflared,
>    gh; BSL 1.1 for our own binaries. All bundled in `bin/`.
> 3. **Cutover compat for v0.1.15 dropped** — fleet is ~1 user
>    (lcf), so the duplicate launcher at tarball root is gone.
>    Clean .app + bin/ tarball layout. Drops Decision #11.
...
1. **Wizard truthfully waits for services.** New launcher HTTP
   ...
   playground is healthy. Wizard exits on Open click.
...
5. **`v0.0.8 → v0.0.9` migration runs in `extract.rs`**: detect
   ...
   fallback), unload old autostart, remove old binary, extract
   split-destination, strip quarantine, re-register autostart,
   `open -a "Friday Studio"`.
...
26 numbered decisions (§Decisions); 12 risk callouts (§Risks);
5 verification items to check on test hardware during
implementation.
...
- `starting` → `healthy`: readiness probe (HTTP 200 against
  `127.0.0.1:<port><path>`) succeeds. `since_secs` resets.
...
- `tools/friday-launcher/healthsvc.go` (new — chi router, handlers,
  RWMutex cache, SSE channel fan-out, state machine)
...
let backoff_deadline = Instant::now() + Duration::from_secs(10);
loop {
...
retry succeeds. After 10s of failed retries, surface
`HealthEvent::Unreachable` to the wizard so it can show "could
not connect to launcher" with a "View logs" button. 10s is
generous — a launcher that hasn't bound port 5199 in 10s is
...
- `apps/studio-installer/src-tauri/src/commands/wait_health.rs` (new —
  SSE relay; backoff loop; staged 60s/90s/150s/+optional deadlines)
...
7. **Pre-flight check via `osascript`: missing/corrupt binaries →
   dialog → Quit.** At launcher startup, BEFORE `systray.Run`,
   verify each supervised binary exists at the expected path AND
   has the exec-bit set. If any are missing, render a native
   dialog by spawning `osascript`:
   ...
   func showMissingBinariesDialog(missing []string) string {
       ...
       script := fmt.Sprintf(
           `display dialog %q with title "Friday Studio" buttons {"Quit", "Open download page"} default button "Open download page" with icon caution`,
           msg)
       cmd := exec.Command("osascript", "-e", script)
       out, _ := cmd.CombinedOutput()
       // osascript returns "button returned:Quit" or
       // "button returned:Open download page"; parse the suffix.
       if strings.Contains(string(out), "Open download page") {
           return "open"
       }
       return "quit"
   }
   ...
   Why osascript (not cgo NSAlert) for pre-flight: NSAlert.runModal
   requires a running NSApp, and NSApp comes up inside `systray.Run`.
   Pre-flight runs BEFORE that. osascript spawns AppleScript which
   has its own NSApp instance, no interaction with ours. The
   confirmation modal for Quit (Issue 6) fires AFTER systray.Run
   has NSApp up, so it can use cgo NSAlert for nicer styling —
...
- `scripts/build-studio.ts` — substantial rewrite:
  ...
  - License extraction: when downloading nats-server's release
    tarball, extract `LICENSE` from
    `nats-server-vX.Y.Z-platform/LICENSE` to `bin/nats-server-license`.
    Same for cloudflared (its release tarball's `LICENSE`) and
    gh (its release tarball's `LICENSE`). Copy
    repo-root `LICENSE` (BSL 1.1) to `bin/LICENSE`.
...
- `tools/friday-launcher/preflight_dialog_darwin.go` (new — osascript
  wrapper; returns "quit" or "open")
- `tools/friday-launcher/preflight_dialog_windows.go` (new —
  MessageBoxW for Windows parity)
...
- `apps/studio-installer/src-tauri/src/commands/extract.rs`
  ...
  strip post-extract; `archive.entries()` iteration)
...
3. **Disable old autostart.** The LaunchAgent label
   ...
   exe path.
4. **Clean up old flat layout.** Remove the v0.0.8 binaries
   that lived directly under `~/.friday/local/`:
   ...
   The v0.0.9 versions of these binaries land in
   `~/.friday/local/bin/` in step 5; the old paths must be gone
   so a future `SweepByBinaryPath(~/.friday/local/bin)` doesn't
   look at stale matches under the wrong directory. Preserve
   `~/.friday/local/{pids,logs,state.json,.env}` — those stay.
5. **Extract new tarball with split-destination.**
   - `Friday Studio.app/` → `/Applications/`
   - `bin/` → `~/.friday/local/bin/` (binaries + LICENSE files)
6. **Strip quarantine xattr from `/Applications/Friday Studio.app`.**
   Pre-empts Gatekeeper's "downloaded from the internet" prompt
   on first launch; .app is signed + notarized so this is safe.
7. **Re-register autostart.** New LaunchAgent points at
...
8. **First launch.** Spawn the new launcher via `open -a "Friday
...
- **Port 5199** — launcher's HTTP health server. `GET
  ...
  /api/launcher-shutdown`. Loopback only.
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  to .app + binaries split. See
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v5.md` §
  Migration.
...
5. **Test order:** ship v0.1.16 installer first; manually
   ...
   v0.1.16 → it pulls v0.0.9 manifest → migration logic cleans
   up v0.0.8 flat layout → unpacks .app to /Applications +
   bin/ to ~/.friday/local/bin → strips quarantine xattr →
   launcher exposes /api/launcher-health → wizard's wait step
   subscribes with capped-backoff retry + staged 60s/90s/+60s
...
- **HTTP port 5199 collision**: hardcoded port for the launcher's
  ...
  startup fails with a clear error and we move it.
...
- **SSE early-connect race**: addressed by capped-backoff retry
  in `wait_health.rs` (200ms, 400ms, 800ms… max 2s, total 10s
  before failing). Fast machines see one retry at most; slow
  spawn paths still succeed.
...
- **osascript availability on stripped-down macOS images**:
  ...
  AppleScript may be disabled by MDM policy — pre-flight falls
  back to logging the error + exiting if `osascript` returns
  non-zero. Worst case: silent exit on broken install. Documented.
...
- **Quarantine xattr strip failure**: if `xattr -dr` fails (rare;
  ...
  regardless.
- **Upstream LICENSE-file presence in third-party release
  tarballs**: scripts/build-studio.ts extracts LICENSE files
  from nats-server, cloudflared, and gh release archives at
  pinned versions. If a future bump to one of those pins lands
  a release that omits LICENSE (extremely unlikely for these
  projects but worth flagging), the build fails loudly rather
  than silently skipping the file. Pin a known-good version
  with LICENSE present; build asserts file exists post-extract.
...
counterpart, `tools/friday-launcher/integration_test.go`
...
with capped-backoff retry; staged 60s/90s/+60s deadline;
per-service checklist; exit-on-Open-click. Plus extract
running-count UI and the verify subtitle flip-order fix.
...
logic comes in Stack 3, keep this commit flat-extract for now),
`apps/studio-installer/src-tauri/Cargo.toml`
(`eventsource-client`),
...
elevation, quarantine xattr strip, pre-flight via osascript,
binDir resolution for .app context, v0.0.8 → v0.0.9 migration.
Touches the build pipeline and extract.rs's destination logic.

Files: `scripts/build-studio.ts` (.app staging + duplicate
launcher at root), `.github/workflows/studio-build.yml`
(codesign --deep + notarization), `tools/friday-launcher/main.go`
(binDir + pre-flight call site), `tools/friday-launcher/preflight.go`
+ `preflight_dialog_{darwin,windows}.go` (new),
...
| 2 | `apps/studio-installer/src-tauri/src/commands/wait_health.rs` (unit tests) | Backoff loop succeeds when launcher is unreachable for first 3 retries then becomes available; surfaces `Unreachable` after 10s of failures. Mock SSE server. |
...
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend) | Migration: pre-existing `~/.friday/local/friday-launcher` is removed; pre-existing `~/.friday/local/Friday Studio.app/` is removed; LaunchAgent plist is unloaded then overwritten to new path; `xattr` shows no `com.apple.quarantine` after extract (verify via `xattr -p`). |
| 3 | Manual QA on a v0.0.8-installed machine | End-to-end migration per the v6 §Test before shipping checklist (LaunchAgent path, Spotlight indexing, silent first launch, reboot autostart). |
...
## Decisions (confirmed 2026-04-27, v5 pass)
...
19. **SSE relay uses capped exponential backoff for early-connect**
    — 200ms, 400ms, 800ms… max 2s; up to 10s total before
...
22. **CLAUDE.md gets a Friday Studio platform layout section** —
    enumerates port 5199, bundle id, wait deadline, install paths,
    and migration reference for fast discovery by future
...
25. **Supervised binaries live in `~/.friday/local/bin/`** —
    ...
    - Migration step: `rm
      ...
      (the v0.0.8 flat layout) before extract
...
26. **License files bundled in `bin/`** — Apache 2.0 / MIT
    ...
    BSL 1.1 for our own:
    - `bin/nats-server-license` (Apache 2.0, extracted from
      upstream release tarball)
    - `bin/cloudflared-license` (Apache 2.0, ditto)
    - `bin/gh-license` (MIT, ditto)
    - `bin/LICENSE` (BSL 1.1, copied from repo root)
    `scripts/build-studio.ts` extracts the third-party LICENSE
    files from the same release archives it already downloads
    for the binaries.
