<!-- v12 - 2026-04-27 - Generated via /improving-plans from 2026-04-27-installer-launcher-ux-fixes.v11.md -->

# Installer + Launcher UX Fixes (v12)

**Date:** 2026-04-27
**Branch:** declaw (per Atlas rule)
**Status:** Plan — implementation-ready
**Supersedes:** v11 (`2026-04-27-installer-launcher-ux-fixes.v11.md`)

> **What v12 changes from v11 (real design changes, not polish):**
> Three small fixes addressing concrete correctness/ergonomics
> gaps that survived eleven prior reviews:
> 1. **Concurrent-shutdown CAS gate hoisted into
>    `performShutdown`.** v11's `cache.shuttingDown.CompareAndSwap`
>    only gates the HTTP path; tray-Quit + NSApp will-terminate +
>    HTTP POST could each spawn `performShutdown` goroutines
>    concurrently. v12 moves the CAS into `performShutdown`
>    itself so all trigger paths are gated by a single choke
>    point.
> 2. **Pre-flight + bind-failure errors write a diagnostic log
>    to `os.TempDir()`.** When the osascript dialog fires, the
>    user's only artefact today is the message body. v12 adds
>    a `friday-launcher-startup.log` in `os.TempDir()` (always-
>    writable, even on a half-broken install) with the missing
>    binaries / port-in-use details, OS version, exe path. The
>    dialog body gains a line pointing at the log path so the
>    user can attach it to a support request.
> 3. **`open -a "Friday Studio"` → `open -b
>    "ai.hellofriday.studio-launcher"`.** Bundle-ID resolution is
>    deterministic; name-based `-a` could pick a stale .app on
>    disk (e.g. an old copy in ~/Downloads). Decision #3 already
>    commits to the bundle ID — using it for `open` is just
>    consistency. Updates Decision #29's plist payload, migration
>    steps 9 + 10, and the test matrix.
>
> All v11 design decisions roll forward unchanged: HTTP shutdown
> response table (cross-cutting §), Decision #32 auth-walled
> caveat, Decision #31 `.bak` cleanup. Plus everything earlier:
> pinned LICENSE URLs, port-5199 dialog, autostart via the
> launcher's bundle ID through LaunchServices, staging-then-swap
> migration, 20s SSE-connect deadline.
...
34 numbered decisions (§Decisions); 14 risk callouts (§Risks);
...
7. **Pre-flight check via `osascript`: missing/corrupt binaries OR
   ...
   (Decision #34, NEW in v12). `~/.friday/local/logs/` may not
   exist on a half-broken install, so the log goes to
   `os.TempDir()` instead — always-writable on macOS:
...
4. **Pre-extract cleanup of stale staging + backup dirs.**
   ...
   The `.bak` cleanup (added in v11) covers the crash-between-
   step-6-and-step-7 case, where the migration renamed the old
   `.app` to `.bak` but didn't get to remove it — Spotlight
   would otherwise index both the live `.app` and the orphan
   `.bak`, returning two results for `Cmd+Space → "Friday"`.
...
- **Negative test (.bak orphan recovery):** simulate a crash
  between migration step 6 (rename live `.app` to `.bak`) and
  step 7 (remove `.bak`) by manually creating
  `/Applications/Friday Studio.app.bak` before running the
  installer. After install, verify the `.bak` is gone and
  `mdfind -name "Friday"` returns exactly one .app result.
  Exercises the v11 extension to Decision #31.
...
- **Migration v0.0.8 → v0.0.9 layout** — installer detects the
  ...
  `docs/plans/2026-04-27-installer-launcher-ux-fixes.v12.md`
  § Migration.
...
| 3 | `apps/studio-installer/src-tauri/src/commands/extract.rs` (extend; **NEW in v10**, extended in v11) | Pre-extract cleanup of stale staging dirs (Decision #31): seed `~/.friday/local/bin.new/`, `/Applications/Friday Studio.app.new`, AND `/Applications/Friday Studio.app.bak` with partial-content fixtures that simulate prior crashed runs; invoke extract; assert all three stale paths are removed at step 4 before the new extraction writes anything. End state matches a clean install with no stale files. |
...
| 3 | `tools/friday-launcher/preflight_dialog_darwin_test.go` (new; **NEW in v12**) | Startup-error log path (Decision #34): call `showMissingBinariesDialog(["foo", "bar"])` with osascript stubbed to return "Quit"; assert `os.TempDir()/friday-launcher-startup.log` exists, contains `"missing_binaries: foo,bar"`, contains the exe path + OS arch, and the dialog body string returned to the caller embeds the log path. Same for `showPortInUseDialog`. Append-mode write: invoke twice, assert both entries are in the file. |
...
## Decisions (confirmed 2026-04-27, v12 pass)
...
31. **Pre-extract cleanup of stale staging + backup dirs** (NEW
    ...
    **v11 extension:** the cleanup also wipes
    `/Applications/Friday Studio.app.bak`. Migration step 6
    creates the `.bak` (renaming the old `.app` aside before
    swapping the new one in); step 7 normally removes it. A
    crash between step 6 and step 7 leaves the `.bak` orphan
    on disk, which Spotlight could index alongside the live
    `.app` — confusing UX where `Cmd+Space → "Friday"` returns
    two results. v11's pre-extract cleanup wipes all three
    paths in one block:
    ```rust
    let _ = fs::remove_dir_all("/Applications/Friday Studio.app.new");
    let _ = fs::remove_dir_all("/Applications/Friday Studio.app.bak");
    ...
    migration fully crash-recoverable: the only states observable
    on disk after any failure are "v0.0.8 layout intact" or
    "v0.0.9 layout live" — never a half-written hybrid, never
    a stale `.bak` hanging around.
...
34. **Pre-flight + bind-failure errors write a diagnostic log
    to `os.TempDir()`** (NEW in v12) — when the osascript
    dialog fires for missing binaries or port-in-use, the user's
    only artefact today is the dialog body. They click Quit and
    have nothing to share with support beyond "Friday Studio
    wouldn't start". v11's normal log path
    (`~/.friday/local/logs/launcher.log`) requires the logs dir
    to exist, which a half-broken install might lack. v12 writes
    to `os.TempDir() + "/friday-launcher-startup.log"` —
    always-writable on macOS — with one entry per failed startup
    attempt (open in append mode so repeated failures
    accumulate). Each entry includes timestamp, error reason,
    bin_dir, exe path, OS+arch. The dialog body gains a
    `"Diagnostic log: <path>"` line so the user can copy and
    attach it. Best-effort: if the `OpenFile` call fails, the
    dialog still shows (without the log path). Same path is
    used by both pre-flight (missing binaries) and bind-failure
    (port-in-use) variants.
