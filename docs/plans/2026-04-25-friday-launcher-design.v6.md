<!-- v6 - 2026-04-25 - Generated via /improving-plans from docs/plans/2026-04-25-friday-launcher-design.v5.md -->

# Friday Launcher — Design (v6)
...
Two paths converge on `enableAutostart()`:

1. **Goroutine E — first-run self-registration.** On startup, the
   launcher reads `~/.friday/local/state.json`. If the file is missing
   or `autostart_initialized != true`, the launcher calls
   `enableAutostart()` (writes the OS-native autostart entry pointing
   at `os.Executable()` with `--no-browser` as the arg), sets
   `autostart_initialized = true`, writes the file back, and continues
   normal startup. This makes the install→tray→relaunch-on-reboot
   experience work with zero installer involvement.
2. **Tray menu "Start at login"** — a checkbox menu item bound to
   `enableAutostart()` / `disableAutostart()`. Initial check state
   read from the actual autostart entry's existence at tray-build
   time. Toggling persists immediately.
...
func enableAutostart() error {
    exe, _ := os.Executable()
    plist := fmt.Sprintf(plistTemplate, "ai.hellofriday.studio", exe, "--no-browser")
    path := filepath.Join(os.Getenv("HOME"), "Library/LaunchAgents/ai.hellofriday.studio.plist")
    return os.WriteFile(path, []byte(plist), 0644)
}
func disableAutostart() error { /* os.Remove(path) */ }
func isAutostartEnabled() bool { /* os.Stat(path) == nil */ }
```
...
// disable: registry.DeleteValue
// isEnabled: registry.GetStringValue
```
