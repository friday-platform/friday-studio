<!-- v7 - 2026-04-25 - Generated via /improving-plans from docs/plans/2026-04-25-friday-launcher-design.v6.md -->

# Friday Launcher — Design (v7)
...
ShutDownParams: types.ShutDownParams{
    ...
    Signal:          int(syscall.SIGTERM), // numeric, NOT the string "SIGTERM"
},
```
...
**`Signal` is `int`, not `string`.** process-compose's
`ShutDownParams.Signal` field is typed as `int` (a numeric
`syscall.Signal` value, e.g. `syscall.SIGTERM == 15`), not a
human-readable string. Earlier drafts used `Signal: "SIGTERM"` which
would not compile. Always pass `int(syscall.SIGTERM)`.
