<!-- v3 - 2026-04-23 - Generated via /improving-plans from docs/plans/2026-04-23-studio-artifact-design.v2.md -->
...
- **Trust contract:** Callers never see GCS internals. `offset=0, length=-1` is a full read. `ErrNotFound` is the only domain error callers need to handle. `ObjectMeta.ETag` is already a valid HTTP ETag string.
...
apps/studio-artifact/
  ...
  service/
    ...
    storage.go         — StorageClient: Stat(), Open(); ObjectMeta.ETag from attrs.Etag
...
- **Unit tests on `handler.go`**: inject a `fakeStorage` that returns canned
  ...
  `If-Modified-Since`), HEAD (assert no GCS `Open` call, correct headers
  present including `ETag` from `ObjectMeta`).
