<!-- v6 - 2026-04-23 - Generated via /improving-plans from docs/plans/2026-04-23-studio-installer-design.v5.md -->
...
- **`launch.rs`**: process that exits within 100ms → `Err` naming the process and exit code; atlasd health endpoint not reachable within 30s → `Err("atlasd did not become healthy within 30s")`; link health endpoint not reachable → `Err("link did not become healthy within 30s")`; agent-playground spawn not attempted until both backend health checks pass; all five processes stay alive, backends healthy, port 5200 opens → `Ok` with PID files written for all five processes; `check_running_processes()` returns `true` immediately after successful `launch_studio()` (PID files exist and processes are live); port 5200 never opens within 30s → `Err` with timeout message.
