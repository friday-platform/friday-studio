<!-- v2 - 2026-04-26 - Generated via /improving-plans from docs/plans/2026-04-26-webhook-tunnel-go-rewrite-design.md -->
...
3. **Rewrite webhook-tunnel** as a Go binary at `tools/webhook-tunnel/`,
   importing `pkg/processkit`, embedding `webhook-mappings.yml` and the
   diceware wordlist, managing cloudflared as a subprocess (same shape as
   the current TS code, but with goroutines + channels instead of
   EventEmitter).
...
10. As a release engineer, I want webhook-tunnel built by the same
    `compileGo` pipeline as pty-server and friday-launcher so the build
    matrix has one Go-binary code path, not two.
11. As a release engineer, I want webhook-tunnel signed in the same codesign
    loop so the macOS notarization step needs no special cases.
12. As an atlas dev working on Go code, I want the launcher's process-kill
    helpers in a shared package so I don't copy-paste them when I write the
    next supervised binary.
13. As an atlas dev, I want a single `go test ./...` from the repo root to
    cover every Go package including the launcher so test coverage isn't
    split across modules I might forget to run.
14. As an atlas dev running `go mod tidy`, I want one module graph to think
    about, not two.
15. As a webhook integrator, I want to override the bundled
...
16. As a webhook integrator debugging signature failures, I want the same
...
17. As an oncall during a tunnel outage, I want the auto-reconnect with
...
18. As an oncall, I want the two-axis health probe (process alive + edge
...
19. As a launcher author, I want webhook-tunnel's binary to live in the
...
20. As a future Go-binary author, I want a stable `processkit` API so my
...
21. As a Windows user, I want webhook-tunnel's child cloudflared process
    to die with webhook-tunnel even on hard kill so I don't accumulate
    orphan cloudflared processes after crashes.
22. As a Unix user, I want SIGTERM to webhook-tunnel to propagate to
    cloudflared via process group so graceful shutdown actually shuts
    cloudflared down too.
23. As an installer running with checksum-pinned downloads, I want the
    in-Go cloudflared fallback to verify the SHA-256 of the downloaded
    binary against a pinned hash so a compromised CDN can't ship me a
    bad binary.
...
3. **`feat(webhook-tunnel): rewrite in Go, drop Deno build`** (single
   ...
   - Internal sub-packages: `tunnel/`, `provider/`, `forwarder/`,
     `passphrase/`, plus `cloudflared/` for the discovery + download
     fallback.
...
- **Cloudflared discovery has four tiers, in order:**
  ...
  4. **In-Go HTTPS fallback download** to `~/.atlas/bin/`. On first run
     ...
     `runtime.GOARCH`. Verify the SHA-256 against a pinned hash table
     embedded in the binary (one entry per supported platform per pinned
     cloudflared version). Show progress on stderr. Cache for next run.
     This preserves the zero-config dev experience the npm `cloudflared`
     package provided.
...
- **Trust contract:** Callers pass already-buffered body bytes and
  ...
  adding a new provider is a YAML change, not a code change. Crucially,
  the byte-slice signature pins the one-shot-body contract — there is no
...
- **Hides:** atlasd URL formatting; status code conventions
  ...
  rewritten on the way through (Host, Content-Length, hop-by-hop headers
  per RFC 7230).
...
- **Interface:** `Resolve(ctx) → (path string, error)`. Implements the
  four-tier discovery + download fallback described above.
- **Hides:** Discovery order; the per-platform cloudflared release URL
  pattern; SHA-256 verification; the cache layout under `~/.atlas/bin/`;
  download progress reporting; concurrent-call coalescing (if two
  goroutines call `Resolve` simultaneously on a missing-binary machine,
  only one download fires).
- **Trust contract:** Caller asks for "a usable cloudflared binary" and
  gets back a path it can hand to `exec.Command`. The pinned hash table
  inside this package is the only place to update on cloudflared version
  bumps.
...
- **Modules tested:**
  ...
  - `cloudflared`: per-platform release URL formatting; SHA-256 mismatch
    rejection (write a wrong-hash file to the cache, assert `Resolve`
    rejects + redownloads); discovery tier ordering (fake all four
    locations, prove the right one wins).
...
- The in-Go cloudflared download fallback exists for the dev-machine case
  ...
  sibling binary so the fallback never fires. The pinned hash table for
  fallback downloads needs maintenance when cloudflared upstream releases:
  a follow-up sweep can also bump the bundled cloudflared version in the
  studio tarball.
...
- The HMAC-body-once contract has a subtle implementation hazard: HTTP/1.1
  ...
  `io.ReadAll(req.Body)` (or equivalent) and only proceed once the read
  completes — not stream the body to the HMAC writer in parallel with
  parsing, which would re-introduce the double-read problem in a more
  subtle form.
