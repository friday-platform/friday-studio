<!-- v4 - 2026-04-25 - Generated via /improving-plans from docs/plans/2026-04-25-friday-launcher-design.v3.md -->

# Friday Launcher — Design (v4)
...
#### Goroutine layout (mandatory)

`fyne.io/systray` calls `runtime.LockOSThread()` at package `init()` time
and `systray.Run()` blocks the calling goroutine permanently
(`[NSApp run]` on macOS). `process-compose`'s `runner.Run()` is also
blocking. They cannot share a goroutine. The launcher's `main()` MUST
look like:

```
main goroutine    : systray.Run(onReady, onExit)        // blocks until Quit
  onReady spawns:
    goroutine A   : runner.Run()                         // process-compose loop
  ...
  onExit          : orderly-shutdown handler (calls runner.ShutDownProject())
```

The signal handler routes:
- `SIGTERM` / `SIGINT` → orderly shutdown via `systray.Quit()` (which fires `onExit`)
- `SIGUSR1` (Unix only) → call `pkg/browser.OpenURL("http://localhost:5200")` (the "wake" path used by single-instance handling on macOS / Linux)
...
`app.ProjectOpts` has **private** fields; struct literals do NOT compile.
The launcher MUST use the builder methods:
```go
opts := app.NewProjectOpts().
    WithProject(project).
    WithIsTuiOn(false).
...
every 2 s and renders one of three states. The predicate is
process-compose's own typed methods, NOT a hand-rolled "is_ready=true"
string check (which would not compile against the real type).
...
- Menu items:
  ...
  - "Restart all" → `runner.RestartProcess(name)` for each supervised
    process (loops over the Project's Processes map)
  - "View logs" → opens `~/.friday/local/logs/` in OS file browser
  - "Quit" → `systray.Quit()` (which fires onExit → orderly shutdown)
...
- **Hides:** that process-compose is the supervisor under the hood;
  ...
  the second-launcher-instance signal-handshake.
...
- **Interface:** `app.NewProjectOpts()` builder + `WithProject(p)` etc.,
