...
ipcMain.handle('install-atlas-binary', async () => {
    try {
        ...
        if (process.platform === 'win32') {
            ...
            installPath = path.join(process.env.ProgramFiles || 'C:\\Program Files', 'Atlas', 'atlas.exe');
            ...
            if (process.platform === 'darwin') {
                ...
                try {
                    ...
                    execSync(`osascript -e 'do shell script "cp '"'"'${binarySource}'"'"' '"'"'${installPath}'"'"' && chmod 755 '"'"'${installPath}'"'"'" with administrator privileges'`);
                    
                } catch (execError) {
                    // Provide more detailed error information
                    return { success: false, error: `Installation failed: ${execError.message}. Binary source: ${binarySource}, Install path: ${installPath}` };
                }
            } else {
