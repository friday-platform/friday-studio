...
// Everything must be self-contained - no imports allowed
...
interface ElectronAPI {
  ...
  createAtlasDir: () => Promise<IPCResult>;
  checkExistingApiKey: () => Promise<ApiKeyCheckResult>;
  saveAtlasNpxPath: () => Promise<IPCResult>;
...
// Global electronAPI injected by preload script
...
(() => {
  ...
  class AtlasInstaller {
    ...
    private async loadPlatformInfo(): Promise<void> {
      try {
        this.platformInfo = await electronAPI.getPlatform();
    ...
    private initializeEventListeners(): void {
      ...
      if (electronAPI?.onInstallationProgress) {
        electronAPI.onInstallationProgress((message: string) => {
    ...
    private async loadLicenseText(): Promise<void> {
      try {
        const licenseText = await electronAPI.getEulaText();
    ...
    private async hasExistingCredentials(): Promise<boolean> {
      try {
        const result: ApiKeyCheckResult = await electronAPI.checkExistingApiKey();
      ...
      const steps: InstallationStep[] = [
        {
          ...
          action: () => electronAPI.createAtlasDir(),
        },
        {
          progress: 28,
          message: "Configuring NPX path for MCP servers...",
          action: () => electronAPI.saveAtlasNpxPath(),
        },
        {
          progress: 42,
          message: "Installing Atlas binary...",
          action: () => electronAPI.installAtlasBinary(),
        },
        {
          progress: 56,
        ...
        { progress: 70, message: "Setting up PATH...", action: () => electronAPI.setupPath() },
        {
          progress: 84,
      ...
      if (atlasKey?.trim()) {
        ...
        if (this.validateAtlasKey(atlasKey)) {
          try {
            ...
            const saveResult = await electronAPI.saveAtlasKey(atlasKey.trim());
            return saveResult;
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    private async manageDaemon(): Promise<IPCResult> {
      try {
        ...
        const binaryCheck: BinaryCheckResult = await electronAPI.checkAtlasBinary();
