...
/**
 * Workspace Simulator CLI — interactive harness for the planner graph pipeline.
 *
 * Usage:
 *   deno task sim "prompt"                     Full pipeline
 *   deno task sim "prompt" --stop-at=plan       Stop after Phase 3
 *   deno task sim "prompt" --stop-at=fsm        Stop after compilation
...
import { createDirectMCPExecutor } from "./harness/direct-executor.ts";
import { type ExecutionReport, runFSM } from "./harness/run-fsm.ts";
...
Usage:
  ...
  deno task sim "prompt" --stop-at=plan      Stop after planning phase
...
// ---------------------------------------------------------------------------
// Pipeline: prompt → plan → compile → run
// ---------------------------------------------------------------------------
...
writeFileSync(
  ...
  JSON.stringify(
    {
      prompt,
      flags: { stopAt: values["stop-at"], real: values.real },
...
try {
  // ── Plan generation via buildBlueprint() ───────────────────────────────
  console.log("\nBuilding blueprint...");
  const {
    blueprint: plan,
    clarifications,
    credentials,
    readiness,
  } = await buildBlueprint(prompt, { mode: "workspace", logger: consoleLogger });
  ...
  if (!readiness.ready) {
    const missingKeys = readiness.checks
      .flatMap((c) => c.checks)
  ...
  // Save blueprint as phase3.json
  writeFileSync(join(runDir, "phase3.json"), JSON.stringify(plan, null, 2));

  if (values["stop-at"] === "plan") {
    writeSummary(runDir, plan, []);
    console.log(`\n--- Stopped at: plan ---`);
    console.log(`Run saved: ${runDir}`);
    exit(0);
  }

  // ── Compile FSM (fatal per-job) ────────────────────────────────────────
  console.log("\nCompiling FSM...");
  const fsms: FSMDefinition[] = [];
  const promptJobWarnings: Array<{ jobId: string; warnings: CompileWarning[] }> = [];
  ...
  for (const job of plan.jobs) {
    ...
    promptJobWarnings.push({ jobId: job.id, warnings: result.value.warnings });
    console.log(
      `  ${job.id}: ${Object.keys(result.value.fsm.states).length} states, ${Object.keys(result.value.fsm.functions ?? {}).length} functions`,
    );
  }

  const promptWarningOutput = formatCompilerWarnings(promptJobWarnings);
  if (promptWarningOutput) {
    console.log(`\n${promptWarningOutput}`);
  }

  writeFileSync(join(runDir, "fsm.json"), JSON.stringify(fsms, null, 2));

  // ── Assemble workspace.yml (non-fatal) ─────────────────────────────────
  let workspaceYamlGenerated = false;
  if (fsms.length > 0) {
    try {
      const phase1 = { workspace: plan.workspace, signals: plan.signals, agents: plan.agents };
      const yaml = buildWorkspaceYaml(phase1, plan, fsms, credentials.bindings);
      writeFileSync(join(runDir, "workspace.yml"), yaml);
      workspaceYamlGenerated = true;
      console.log(`\nworkspace.yml generated`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`\nworkspace.yml assembly failed (non-fatal): ${msg}`);
    }
  }

  // ── Run FSM execution (unless --stop-at=fsm) ──────────────────────────
  let executionReports: ExecutionReport[] | undefined;
  let promptRealExecutorHandle:
    | { executor: Parameters<typeof runFSM>[0]["agentExecutor"]; shutdown: () => Promise<void> }
    | undefined;
  if (values["stop-at"] !== "fsm" && fsms.length > 0) {
    if (values.real) {
      console.log("\nReal mode — agents will execute directly with MCP tools");
      promptRealExecutorHandle = createDirectMCPExecutor({ plan });
    }

    const promptFinalExecutor = promptRealExecutorHandle?.executor;

    // Build list of FSMs to run
    const fsmJobPairs: Array<{ fsm: FSMDefinition; job: (typeof plan.jobs)[number] }> = [];
    for (const fsm of fsms) {
      const job = plan.jobs.find((j) => j.id === fsm.id);
      if (job) fsmJobPairs.push({ fsm, job });
    }

    executionReports = [];
    try {
      for (const { fsm, job } of fsmJobPairs) {
        console.log(`\n--- Running FSM: ${fsm.id} (${values.real ? "real" : "mock"}) ---`);
        const report = await runFSM({
          fsm,
          plan,
          triggerSignal: job.triggerSignalId,
          agentExecutor: promptFinalExecutor,
        });
        executionReports.push(report);
        printExecutionSummary(fsm.id, report);
        if (!report.success) {
          throw new Error(report.error ?? `FSM execution failed for ${fsm.id}`);
        }
      }
    } finally {
      await promptRealExecutorHandle?.shutdown();
    }

    writeFileSync(join(runDir, "execution-report.json"), JSON.stringify(executionReports, null, 2));
  }

  writeSummary(runDir, plan, fsms, workspaceYamlGenerated);

  if (values["stop-at"] === "fsm") {
    console.log(`\n--- Stopped at: fsm ---`);
  }
  ...
  // Write partial summary if we have enough context
...
function topoSortTiers(steps: Array<{ id: string; depends_on: string[] }>): string[][] {
  ...
  // Fallback: cycle detected — return input order, one per tier
