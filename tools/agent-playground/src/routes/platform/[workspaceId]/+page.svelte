  ...
  const jobSummaries = $derived.by((): {
    ...
    return Object.entries(config.jobs).map(([id, job]) => {
      ...
      const triggers = Array.isArray(j?.triggers)
        ? j.triggers.filter((t): t is { signal: string } => typeof t === "object" && t !== null && "signal" in t)
        : [];
    ...
    <div class="row-2">
      {#if topology && (latestSession ?? olderSessions.length > 0)}
        <div class="runs-card card">
          ...
          {#if latestSession}
            <SessionProgressCard session={latestSession} {topology} workspaceId={workspaceId} jobTitles={jobTitleMap} />
          {/if}
          {#if olderSessions.length > 0}
            <div class="compact-runs">
              {#each olderSessions as session (session.sessionId)}
                <a
                  href="/platform/{workspaceId}/sessions/{session.sessionId}"
                  class="compact-run"
                >
          ...
          {/if}
          <a href="/platform/{workspaceId}/sessions" class="view-all-row">
            View all runs
          </a>
        </div>
      {/if}
