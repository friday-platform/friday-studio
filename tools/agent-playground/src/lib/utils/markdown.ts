 ...
 */
function parseMarkdownToAST(text: string): ASTNode | null {
  if (!text) return null;
 ...
 */
function cleanMarkdownSyntax(node: ASTNode): string {
 ...
 */
function extractLinkData(content: string): { text: string; href: string } {
 ...
 */
function astToHTML(node: ASTNode | null, parentType: string = ""): string {
  if (!node) return "";
