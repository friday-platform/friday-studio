...
<script lang="ts">
  ...
  const variants = $derived.by((): Variant[] => {
    ...
    for (const variant of oneOf) {
      ...
      const opProp = (props as Record<string, unknown>)["operation"];
      if (typeof opProp !== "object" || opProp === null) continue;
      const opConst = (opProp as Record<string, unknown>)["const"];
      if (typeof opConst !== "string") continue;

      const req = v["required"];
      const reqSet = Array.isArray(req)
        ? new Set(req.filter((r): r is string => typeof r === "string"))
        : new Set<string>();
    ...
    if (rawMode) {
      ...
      try {
        ...
        if (typeof parsed === "object" && parsed !== null) {
          ...
          if (typeof op === "string" && variants.some((v) => v.operation === op)) {
            selectedOperation = op;
            const newData: Record<string, unknown> = {};
            for (const [key] of Object.entries(obj)) {
              if (key !== "operation") newData[key] = obj[key];
            }
            formData = newData;
          }
        }
