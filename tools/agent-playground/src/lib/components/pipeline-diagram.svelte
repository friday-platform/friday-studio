...
<script lang="ts">
  ...
  let { topology, selectedNodeId = null, onNodeClick, compact = false, nodeStatusMap = {}, disabledSteps = new Set(), onToggleStep }: Props = $props();
...
{:else if compact}
  <div class="pipeline pipeline--compact" role="img" aria-label="Workspace pipeline diagram">
    {#each pipelineNodes as node, i (node.id)}
      {#if i > 0}
        <svg class="connector connector--compact" width="16" height="8" viewBox="0 0 16 8" aria-hidden="true">
      ...
      >
        {#if nodeStatusMap[node.id]}
          <span class="status-dot status-dot--{nodeStatusMap[node.id]}" aria-label={nodeStatusMap[node.id]}></span>
        {/if}
  ...
  <div class="pipeline" role="img" aria-label="Workspace pipeline diagram">
    {#each pipelineNodes as node, i (node.id)}
      ...
      {#if i > 0}
        ...
        {@const connectorDashed = (prevNode && prevNode.type === "agent-step" && isDisabled(prevNode)) || (node.type === "agent-step" && isDisabled(node))}
        <svg class="connector" class:connector--dashed={connectorDashed} width="24" height="12" viewBox="0 0 24 12" aria-hidden="true">
          <line x1="0" y1="6" x2="18" y2="6" stroke="var(--color-border-1)" stroke-width="1" stroke-dasharray={connectorDashed ? "3 2" : "none"} />
