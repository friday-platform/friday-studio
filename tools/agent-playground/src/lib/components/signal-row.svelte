...
<script lang="ts">
  import { Dialog, DropdownMenu } from "@atlas/ui";
        ...
        {#snippet children()}
          ...
          <DropdownMenu.Content>
            {#if signal.type.toLowerCase() === "http"}
              <DropdownMenu.Item onclick={copySignalUrl}>
                Copy signal URL
              </DropdownMenu.Item>
              ...
              {/if}
            {/if}
            <DropdownMenu.Item onclick={() => goto(`/platform/${workspaceId}/edit?path=signals.${signal.id}`)}>
              Edit configuration
            </DropdownMenu.Item>
          </DropdownMenu.Content>
        {/snippet}
              ...
              <div class="webhook-dialog">
                ...
                {#if cachedTunnelSecret}
                  ...
                  <div class="webhook-url-row">
                    ...
                    <button class="copy-btn" onclick={() => copyToClipboard(cachedTunnelSecret ?? "")}>Copy</button>
                  </div>
                {:else}
                  <p class="webhook-no-secret">
                    No webhook secret configured. Set <code>WEBHOOK_SECRET</code> env var to enable signature verification.
                  </p>
                {/if}
