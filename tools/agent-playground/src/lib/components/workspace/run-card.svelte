...
<script lang="ts">
  import OutputTabs from "$lib/components/shared/output-tabs.svelte";
  ...
  /** Map RunRecord status to ExecutionStatus for child components. */
  function toExecutionStatus(status: RunRecord["status"]): ExecutionStatus {
    switch (status) {
      case "running":
        return { state: "running", stream: new ReadableStream() };
      case "cancelled":
  ...
  const executionStatus = $derived(toExecutionStatus(run.status));
</script>
