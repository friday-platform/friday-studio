...
<script lang="ts">
  ...
  import AgentsStrip from "$lib/components/agents/agents-strip.svelte";
  import DataContractsPanel from "$lib/components/agents/data-contracts-panel.svelte";
  import JobSelector, { type JobInfo } from "$lib/components/workspace/job-selector.svelte";
  ...
  import SignalsPanel from "$lib/components/workspace/signals-panel.svelte";
  ...
  const sessionViewQuery = createQuery(() => {
    const id = latestSession?.sessionId ?? null;
    return {
      ...sessionQueries.view(id),
