...
console.log("Testing direct stream API...\n");
...
try {
  ...
  // 3. Trigger the conversation workspace with streamId
  console.log("3. Triggering conversation...");
  setTimeout(async () => {
    const triggerResponse = await fetch("http://localhost:8080/api/streams", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        streamId: streamId,
        message: "Hello! Please use the stream_reply tool.",
        userId: "test-user",
      }),
    });

    if (!triggerResponse.ok) {
      throw new Error(`Trigger failed: ${triggerResponse.status} ${triggerResponse.statusText}`);
    }

    const triggerResult = await triggerResponse.json();
    console.log("✓ Conversation triggered:", triggerResult);
  }, 1000);
