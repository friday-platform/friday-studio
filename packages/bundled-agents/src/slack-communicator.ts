...
export const slackCommunicatorAgent = createAgent<string>({
  ...
  handler: async (prompt: string, { tools, logger, abortSignal, stream }): Promise<string> => {
    ...
    const plannerSystem =
      "You are a minimal Slack task planner. Analyze the user's prompt and produce a strict JSON plan for the executor and summarizer. " +
    ...
    const system =
      ...
      "Do not narrate intentions or plans (avoid phrases like 'I'll' or 'Let me'). Output only the result (the summary) without prefacing text. " +
    ...
    try {
      ...
      const executorInstructions = [
        ...
        "- If needsHistory is true, call conversations_history with targetChannel (or ask briefly if null), limit=historyLimit, include_activity_messages accordingly.",
        "- If messageToSend is present, call conversations_add_message with targetChannel and the message.",
        "- Avoid narration; focus on tool calls and minimal final text (the summarizer will produce the user-facing output).",
      ...
      if (!tools || Object.keys(tools).length === 0) {
        return "Cannot complete: Slack tools unavailable. Provide Slack MCP tools to proceed.";
      }
      ...
      const summarizerSystem =
        ...
        "Rules: no narration; output only the final content; be concise and factual; omit unknowns.";
      ...
      return finalText.length > 0 ? finalText : result.text.trim();
    } catch (error) {
