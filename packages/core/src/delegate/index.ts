  ...
  return tool({
    ...
    execute: async (
      ...
      try {
        ...
        // Tool priority + scope discipline. The supervisor's prompt
        ...
        // head; the delegate sub-agent runs from a separate system prompt
        // and would otherwise feel free to reach for unrelated agent_*
        // tools or work around MCP failures via run_code / upsert_agent.
        // Restate the ranking here so the sub-agent inherits the same
        // discipline.
        ...
        // MCP tool error contract. The supervisor returns `reason` to its
        // own LLM, which decides user-facing language; the sub-agent must
        // not paraphrase or translate the upstream error.
