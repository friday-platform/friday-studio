...
describe.skipIf(!CAN_RUN_INTEGRATION)("validate (Real Haiku judge)", () => {
  ...
  it(
    "well-sourced output does not fail (pass or uncertain)",
    { timeout: 20_000 },
    async () => {
      const result = buildResult({
        input: "Look up the user count and primary contact.",
        data:
          "There are 42 users in the system. The primary contact is Alice Smith at TechCorp.",
        toolCalls: [
  ...
  it(
    "unsourced output with no tools called populates verdict.issues from the schema enum",
    { timeout: 20_000 },
    async () => {
      const result = buildResult({
        input: "What is the company headcount?",
        data:
          "According to LinkedIn, the company has 500 employees and was founded in 2014 in Berlin.",
        toolCalls: [],
        toolResults: [],
      });
  ...
  it(
    "passes computed claims that the judge could otherwise miscompute (arithmetic out-of-scope)",
    { timeout: 20_000 },
    async () => {
