...
export function createResourceReadTool(adapter: ResourceStorageAdapter, workspaceId: string) {
  return tool({
    description:
...
export function createResourceWriteTool(adapter: ResourceStorageAdapter, workspaceId: string) {
  return tool({
    description:
...
export function createResourceSaveTool(adapter: ResourceStorageAdapter, workspaceId: string) {
  return tool({
    description:
    ...
    execute: async ({ slug }) => {
      try {
        return await adapter.publish(workspaceId, slug);
      } catch (err) {
        return { error: err instanceof Error ? err.message : String(err) };
      }
    },
...
export function createResourceLinkRefTool(adapter: ResourceStorageAdapter, workspaceId: string) {
  return tool({
    description:
    ...
    execute: async ({ slug, ref }) => {
      try {
        return await adapter.linkRef(workspaceId, slug, ref);
      } catch (err) {
        return { error: err instanceof Error ? err.message : String(err) };
      }
    },
