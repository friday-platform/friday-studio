/**
 * Configuration schemas and types for Atlas
 * This module contains all Zod schemas used for configuration validation
 */
...
// Schema for the complete supervisors configuration (workspace, session, agent)
export const SupervisorsConfigSchema = z.object({
  workspace: SupervisorConfigSchema,
  session: SupervisorConfigSchema,
  agent: SupervisorConfigSchema,
});

// Schema for supervisor defaults file
export const SupervisorDefaultsSchema = z.object({
  supervisors: SupervisorsConfigSchema,
});

// Memory configuration schemas
const MemoryDefaultConfigSchema = z.object({
  enabled: z.boolean(),
  storage: z.string(),
...
const MemoryStreamingConfigSchema = z.object({
  enabled: z.boolean(),
...
const PlanningExecutionConfigSchema = z.object({
...
export const AtlasConfigSchema = z.object({
  ...
  memory: z.object({
    default: MemoryDefaultConfigSchema,
    streaming: MemoryStreamingConfigSchema.optional(),
  ...
  planning: z.object({
    execution: PlanningExecutionConfigSchema,
    validation: PlanningValidationConfigSchema,
  }).optional(),
