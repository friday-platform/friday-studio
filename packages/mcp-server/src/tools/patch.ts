...
import DESCRIPTION from "./patch.txt" with { type: "txt" };
  ...
  execute: async (params, ctx) => {
    ...
    for (const filePath of filesToRead) {
      ...
      if (!path.isAbsolute(absPath)) {
        absPath = path.resolve(Deno.cwd(), absPath);
      }
    ...
    const result =
      `Patch applied successfully. ${changedFiles.length} files changed, ${totalAdditions} additions, ${totalRemovals} removals`;
    const output = result;
