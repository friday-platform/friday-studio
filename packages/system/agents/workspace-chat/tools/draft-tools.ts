...
export function createBoundDraftTools(logger: Logger, workspaceId: string): AtlasTools {
  return {
    begin_draft: tool({
      ...
      inputSchema: z.object({ workspaceId: z.string().optional() }),
      execute: async ({ workspaceId: providedId }: { workspaceId?: string }) => {
        const targetId = providedId ?? workspaceId;
        logger.info("begin_draft tool invoked", { workspaceId: targetId });

        const res = await client.workspace[":workspaceId"].draft.begin.$post({
          param: { workspaceId: targetId },
        });
        ...
        logger.info("begin_draft succeeded", { workspaceId: targetId });
    ...
    publish_draft: tool({
      ...
      execute: async ({ workspaceId: providedId }: { workspaceId?: string }) => {
        const targetId = providedId ?? workspaceId;
        logger.info("publish_draft tool invoked", { workspaceId: targetId });
    ...
    validate_workspace: tool({
      ...
      execute: async ({ workspaceId: providedId }: { workspaceId?: string }) => {
        const targetId = providedId ?? workspaceId;
        logger.info("validate_workspace tool invoked", { workspaceId: targetId });
        ...
        if (draftRes.ok) {
          ...
          logger.info("validate_workspace succeeded (draft)", { workspaceId: targetId });
          return data.report;
        }
        ...
        if (draftRes.status === 409) {
          ...
          if (lintRes.ok) {
            ...
            logger.info("validate_workspace succeeded (live)", { workspaceId: targetId });
            return data.report;
          }
    ...
    discard_draft: tool({
      ...
      execute: async ({ workspaceId: providedId }: { workspaceId?: string }) => {
        const targetId = providedId ?? workspaceId;
        logger.info("discard_draft tool invoked", { workspaceId: targetId });
