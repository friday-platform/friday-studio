  ...
  return {
    ...
    upsert_memory_own: tool({
      description:
        ...
        "The schema technically accepts `'retrieval' | 'dedup' | 'kv'` (Phase 1b), but those backends are not wired into the runtime: writes throw at the adapter layer, `save_memory_entry`/`list_memory_entries` error out, and entries never auto-inject. Do NOT use them. If a user asks for vector search, dedup queues, or key-value lookup, surface the limitation — don't author a store that won't work. " +
