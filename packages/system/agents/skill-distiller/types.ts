import { NamespaceSchema } from "@atlas/config";
import { z } from "zod";

export const SkillDistillerInputSchema = z.object({
  artifactIds: z.array(z.string()).min(1).describe("Artifact IDs containing store material"),
  namespace: NamespaceSchema.optional().describe("Target namespace (falls back to 'friday')"),
...
export interface SkillDistillerOutput {
  ...
  revision: number;
  skill: { name: string; namespace: string; description: string; instructions: string };
  nextStep: string;
}
