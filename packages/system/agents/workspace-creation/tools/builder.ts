...
export class WorkspaceBuilder {
  ...
  /**
   * Automatically discover and add suitable MCP servers for given requirements
   * This ensures consistent MCP registry usage across all workspace creation paths
   */
  async discoverAndAddMCPServers(requirements: string[]): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    try {
      ...
      for (const requirement of requirements) {
        ...
        if (discovery && discovery.confidence >= 0.6) {
          const serverName = discovery.server.id;
          ...
          if (!this.mcpServers.has(serverName)) {
            ...
            if (result.success) {
              addedCount++;
              warnings.push(
                `Auto-discovered MCP server '${serverName}' for requirement: ${requirement}`,
              );
            } else {
              errors.push(
                `Failed to add discovered server '${serverName}': ${result.errors.join(", ")}`,
              );
            }
          }
        } else {
          warnings.push(`No suitable MCP server found for requirement: ${requirement}`);
        }
      }
      ...
      return {
        ...
        warnings:
          addedCount > 0
            ? [`Auto-discovered and added ${addedCount} MCP servers`, ...warnings]
            : warnings,
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      return { success: false, errors: [`MCP discovery failed: ${errorMessage}`], warnings };
    }
  }
