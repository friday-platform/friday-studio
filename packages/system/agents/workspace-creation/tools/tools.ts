...
export function getWorkspaceBuilderTools(builder: WorkspaceBuilder): AtlasTools {
  return {
    ...
    initializeWorkspace: tool({
      ...
      execute: ({ name, description }) => {
        ...
        const result = builder.initialize({ name, description });
        if (!result.success) {
    ...
    discoverAndAddMCPServers: tool({
      ...
      execute: async ({ requirements }) => {
        const result = await builder.discoverAndAddMCPServers(requirements);
        if (!result.success) {
