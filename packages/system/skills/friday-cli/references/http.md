...
- `POST /build` — multipart `files` + optional `entry_point`, `sdk_path`. Runs componentize-py + jco, writes to agents dir, reloads registry. **Build mechanics + authoring: use the sibling `writing-friday-agents` skill at `packages/system/skills/writing-friday-agents/`.**
