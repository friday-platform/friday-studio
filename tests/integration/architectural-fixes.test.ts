...
`;
  ...
  try {
    ...
    assertExists(mergedConfig.supervisorDefaults.supervisors.session, "session supervisor config should exist");
    assertExists(mergedConfig.supervisorDefaults.supervisors.workspace, "workspace supervisor config should exist");
    assertExists(mergedConfig.supervisorDefaults.supervisors.agent, "agent supervisor config should exist");
...
Deno.test("SessionSupervisor - accepts and uses supervisor defaults", () => {
  ...
  assertEquals(supervisorDefaults.supervisors.session.prompts.system, "Test session supervisor system prompt");
});
...
Deno.test("SessionSupervisor - MCP tools architecture uses new structure", () => {
  ...
  const mcpConfigs = supervisor.getMcpServerConfigsForAgent("test-agent", ["file-server", "web-server"]);
...
Deno.test("SessionSupervisor - task preparation uses reasoning engine not hardcoded prompts", async () => {
  ...
  (supervisor as any).generateLLM = async (model: string, system: string, prompt: string, streaming: boolean, metadata: any) => {
...
Deno.test("SessionSupervisor - output formatting adapts library patterns", async () => {
  ...
  assertEquals(formattedTask.includes("## Requirements"), true, "Should include requirements section");
