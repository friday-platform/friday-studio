...
class MockServer {
  ...
  addRoute(path: string, options: any) {
...
Deno.test("Atlas Client E2E - should fetch library items through the complete stack", async () => {
  ...
  try {
    mockServer.addRoute("/api/library", {
      method: "GET",
      handler: (req) => {
        const url = new URL(req.url);
...
Deno.test("Atlas Client E2E - should reuse client instance across modules", async () => {
  ...
  try {
    ...
    globalThis.getAtlasClient = (options?: any) => {
      clientCount++;
      return originalGetAtlasClient(options);
    };

    try {
      // Multiple calls with same options should reuse instance
      await fetchSessions({ port: serverPort });
      await fetchLibraryItems({ port: serverPort });

      // Only 2 clients should be created (one for each unique URL)
