...
Deno.test("Session FSM transitions through planning-executing-evaluating-refining cycle", async () => {
  ...
  assertEquals(states.includes("executingAgents"), true, "Should have executingAgents state");
  assertEquals(states.includes("evaluating"), true, "Should have evaluating state");
...
Deno.test("WorkspaceSupervisor generates execution plan from intent", async () => {
  ...
  supervisor.generateLLM = async (model: string, system: string, prompt: string) => {
    return JSON.stringify({
      phases: [{
        ...
        agents: [
          ...
          { agentId: "agent-2", task: "Transform result", dependencies: ["agent-1"] }
        ],
