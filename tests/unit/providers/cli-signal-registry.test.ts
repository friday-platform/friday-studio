...
Deno.test("CLI Signal Provider - Registry Integration", async (t) => {
  ...
  await t.step("should fail with invalid CLI config", async () => {
    ...
    await assertRejects(() => registry.loadFromConfig(config), Error, "command");
  });
...
Deno.test("CLI Signal Provider - Configuration Schema Integration", async (t) => {
  await t.step("should handle complex CLI configurations", async () => {
    ...
    const config: ProviderConfig = {
      ...
      config: {
        ...
        flags: { verbose: true, force: false, timeout: 300, dryRun: false },
        timeout_ms: 30000,
  ...
  await t.step("should validate configuration schema", async () => {
    ...
    const invalidArgsConfig: ProviderConfig = {
      ...
      type: ProviderType.SIGNAL,
      provider: "cli",
      config: { command: "test", args: "invalid-args-should-be-array" },
    };

    await assertRejects(() => registry.loadFromConfig(invalidArgsConfig), Error, "args");
    ...
    const invalidFlagsConfig: ProviderConfig = {
      ...
      type: ProviderType.SIGNAL,
      provider: "cli",
      config: { command: "test", flags: ["invalid", "flags", "should", "be", "object"] },
    };

    await assertRejects(() => registry.loadFromConfig(invalidFlagsConfig), Error, "flags");
  });
});
