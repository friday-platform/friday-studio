...
function createValidator(overrides: Partial<ConstructorParameters<typeof JeopardyValidator>[0]> = {}) {
  return new JeopardyValidator({
    enabled: true,
    model: "mock-model",
    llmProvider: () => new MockLanguageModelV2({
      // Default: a minimal valid JSON body
      doGenerate: async () => ({
        finishReason: "stop",
        usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 },
        content: [{ type: "text", text: `{"answersTask":true,"completeness":1,"confidence":1,"issues":[],"reasoning":""}` }],
        warnings: [],
      }),
  ...
  const v = createValidator({
    llmProvider: () => new MockLanguageModelV2({
      doGenerate: async () => ({
        finishReason: "stop",
        usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 },
        content: [{ type: "text", text: `{"answersTask":true,"completeness":0.4,"confidence":0.6,"issues":[{"type":"incomplete","description":"missing key fields","severity":"medium"}],"reasoning":"partial"}` }],
        warnings: [],
      }),
    }) as unknown as import("ai").LanguageModel,
  });
  ...
  const v = createValidator({
    llmProvider: () => new MockLanguageModelV2({
      doGenerate: async () => ({
        finishReason: "stop",
        usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 },
        content: [{ type: "text", text: `{"answersTask":true,"completeness":0.9,"confidence":0.9,"issues":[],"reasoning":"good"}` }],
        warnings: [],
      }),
    }) as unknown as import("ai").LanguageModel,
  });
