...
import { JobTriggerMatcher, type JobSpec } from "../../src/core/job-trigger-matcher.ts";
...
Deno.test("JobTriggerMatcher handles different signal types", async () => {
  ...
  const matches = await matcher.findMatchingJobs(differentSignal, payload, jobsWithDifferentSignals);
