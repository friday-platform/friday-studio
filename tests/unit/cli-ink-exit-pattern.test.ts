...
Deno.test("CLI commands should not use immediate Deno.exit() after render()", async () => {
  ...
  for await (const entry of walk(cliCommandsPath, { exts: [".tsx", ".ts"], includeDirs: false })) {
    ...
    for (let i = 0; i < lines.length - 2; i++) {
      ...
      if (
        currentLine.includes("render(") &&
        nextLine.includes("//") &&
        nextNextLine.includes("Deno.exit(")
      ) {
        problematicFiles.push(
          `${entry.path}:${i + 1}-${i + 3} - render() followed by comment then Deno.exit()`,
        );
      }
  ...
  for await (const entry of walk(cliCommandsPath, { exts: [".tsx", ".ts"], includeDirs: false })) {
    ...
    if (!hasJsonAlternative) {
      recommendations.push(
        `${entry.path}: Consider adding --json flag for scripting compatibility`,
      );
    }
