...
Deno.test("Secure cache key generation", () => {
  ...
  expect(createSecurePlanKey("complex-job-name", "another-workspace")).toBe("plan:another-workspace:complex-job-name");
...
Deno.test("Workspace access validation", () => {
  function validateWorkspaceAccess(requestingWorkspaceId: string, supervisorWorkspaceId: string): boolean {
