<!--
  Friday wordmark glyph. Extracted from `sidebar.svelte` (one source,
  one shape) so the welcome / login surfaces can render the same
  mark without duplicating the SVG path.

  Default size matches the sidebar header (24×24); the welcome page
  overrides via `size` for a larger hero treatment.
