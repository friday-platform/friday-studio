...
describe("daemon startup platform models", () => {
  ...
  describe("valid yaml configuration", () => {
    it("boots successfully with anthropic credentials and resolves all roles", async () => {
    ...
    it("boots successfully with groq credentials and resolves labels from groq", async () => {
  ...
  describe("invalid yaml configuration", () => {
    it("throws PlatformModelsConfigError when no credentials are available", async () => {
    ...
    it("throws when malformed provider:model format is configured", async () => {
    ...
    it("throws on unknown provider in user configuration", async () => {
      const { FilesystemAtlasConfigSource } = await import("@atlas/config/server");

      const originalLoad = FilesystemAtlasConfigSource.prototype.load;
      FilesystemAtlasConfigSource.prototype.load = () =>
        Promise.resolve({ models: { labels: "unknownprovider:some-model" } } as unknown as Awaited<
          ReturnType<typeof originalLoad>
        >);
  ...
  describe("no yaml configuration (zero-config)", () => {
    it("boots with default chains when anthropic is credentialed", async () => {
    ...
    it("prefers groq for labels when both groq and anthropic are credentialed", async () => {
      setCredential("GROQ_API_KEY", "test-groq-key");
      setCredential("ANTHROPIC_API_KEY", "test-anthropic-key");
  ...
  describe("LITELLM universal credential", () => {
    it("boots with only LITELLM_API_KEY set (no provider-specific credentials)", async () => {
    ...
    it("allows any provider model when LITELLM_API_KEY is set", async () => {
      const { FilesystemAtlasConfigSource } = await import("@atlas/config/server");
