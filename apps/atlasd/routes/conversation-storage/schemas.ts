...
export const streamIdParamSchema = z
  .object({ streamId: z.string().min(1).describe("Stream ID for conversation operations") })
...
export const storeDataSchema = z
  .object({
    message: z.object({ role: z.enum(["user", "assistant"]), content: z.string() }),
...
export const storeResponseSchema = z
  .object({ success: z.boolean(), messageId: z.string().optional(), error: z.string().optional() })
