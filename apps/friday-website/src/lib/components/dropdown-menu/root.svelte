...
let { children, ...args }: Props & CreateDropdownMenuProps = $props();

const { open } = createContext({ ...args, forceVisible: true, closeOnOutsideClick: true });
</script>
...
<style>
	:global([data-melt-dropdown-menu]) {
		--dropdown-menu-content-padding: var(--size-4);
