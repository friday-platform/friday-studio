...
import type { ClientRegistration, Metadata, OAuthCredential, StorageAdapter } from "../types.ts";
...
export class OAuthService {
  ...
  async completeFlow(
    ...
    // 7. Build synthetic ID
    const id = `oauth:${providerId}:${userIdentifier}`;

    // 8. Build and save OAuth credential
    ...
    const credential: OAuthCredential = {
      ...
      provider: providerId,
      userIdentifier,
      label: userIdentifier, // OAuthCredential.label is optional, defaults to userIdentifier
      secret: {
    ...
    await this.storage.save(credential, userId || "dev");
  ...
  private async doRefresh(credential: OAuthCredential, userId: string): Promise<OAuthCredential> {
    ...
    // 4. Update credential with new tokens
    const updated: OAuthCredential = {
      ...
      metadata: { ...credential.metadata, updatedAt: new Date().toISOString() },
    };

    // 5. Save and return updated credential
    await this.storage.save(updated, userId);
    return updated;
  }
}
