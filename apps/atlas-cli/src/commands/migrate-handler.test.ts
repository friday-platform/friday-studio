 ...
 * JSON-object line and forwards it as the outcome. That contract is
 * load-bearing and unenforced at the type level — anything emitted to
 * stdout AFTER the outcome `console.log` (a stray `logger.info`, a
 * "wrote audit record" line, a telemetry hook) would silently shadow
 * the outcome.
 *
 * These tests pin the invariant directly: for every `--json` code path,
 * parsing the LAST JSON-object line of stdout MUST yield the documented
 * outcome shape — never a logger line, never a half-shape from --list.
 *
 * Mutation resistance: if a maintainer adds `console.log(JSON.stringify
 * ({foo:"trailer"}))` after any of the three outcome emits, the
 * matching test below will fail because the LAST-line parse won't
 * contain the expected `preNats`/`ran`/etc. fields.
 */
describe("--json outcome stdout discipline", () => {
  /** Helper: parse the LAST JSON-object line of capturedStdout (matching
   * what the installer's `last_json_object_line` does). */
  function lastJsonObjectLine(): unknown {
    for (let i = mockState.capturedStdout.length - 1; i >= 0; i--) {
      const line = mockState.capturedStdout[i]?.trim();
      if (!line) continue;
      try {
  ...
  it("post-NATS success: last stdout JSON-object line is the outcome", async () => {
    mockState.preNatsResult = { outcomes: [], aborted: false };
    mockState.postNatsBehavior = "succeed";
    mockState.runMigrationsResult = {
      ran: ["m_20260501_120000_chat_to_jetstream"],
      skipped: [],
      failed: [],
    };
  ...
  it("post-NATS failure: last stdout JSON-object line is the outcome", async () => {
    mockState.preNatsResult = { outcomes: [], aborted: false };
    mockState.postNatsBehavior = "succeed";
    mockState.runMigrationsResult = {
      ran: [],
  ...
  it("pre-NATS aborted: last stdout JSON-object line is the outcome", async () => {
    mockState.preNatsResult = {
      outcomes: [
        {
          id: "stub",
          ...
          target_source: "default",
          duration_ms: 0,
          error: { kind: "unknown", message: "simulated" },
        },
      ],
      aborted: true,
    };
