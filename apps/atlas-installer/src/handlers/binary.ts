...
async function installWebClient(): Promise<void> {
  try {
    // For Tauri, use __dirname to find atlas-binary
    const basePath = path.join(__dirname, "..", "..", "atlas-binary");

    if (!fs.existsSync(basePath)) {
      logger.warn(`atlas-binary directory not found at ${basePath}`);
      return;
    }
    ...
    if (isMac()) {
      ...
      try {
        await createWebClientSymlink();
      } catch (err) {
        logger.warn(`Symlink creation failed but web client installation continues: ${err}`);
      }
    } else if (isWindows()) {
...
async function copyBundledBinaries(): Promise<IPCResult> {
  try {
    ...
    // For Tauri, use __dirname to find atlas-binary
    const sourcePath = path.join(__dirname, "..", "..", "atlas-binary");

    if (!fs.existsSync(sourcePath)) {
      logger.warn(`atlas-binary directory not found at ${sourcePath}`);
...
/**
 * Create symlink for web client in /Applications with admin privileges
 */
async function createWebClientSymlink(): Promise<void> {
  const userAppPath = path.join(os.homedir(), "Applications", "Atlas Web Client.app");
  const systemAppPath = "/Applications/Atlas Web Client.app";

  try {
    // Check if symlink already exists and is correct
    const current = fs.readlinkSync(systemAppPath);
    if (path.resolve("/Applications", current) === path.resolve(userAppPath)) {
      logger.info("Web client symlink already configured");
      return;
    }
  } catch {
    // Not a symlink or doesn't exist, proceed with creation
  }

  try {
    const cmd = `rm -f "${systemAppPath}" && ln -sf "${userAppPath}" "${systemAppPath}"`;
    await safeExec(`osascript -e 'do shell script "${cmd}" with administrator privileges'`);
    logger.info("Created /Applications symlink for Atlas Web Client");
  } catch (err) {
    logger.warn(`Could not create /Applications symlink for web client: ${err}`);
  }
}
