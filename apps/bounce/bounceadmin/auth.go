...
func doOAuthLogin(ctx context.Context) (*oauth2.Token, error) {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case errMsg != "":
			_, _ = fmt.Fprintf(w, "Authentication failed: %s", html.EscapeString(errMsg)) //nolint:gosec
...
func openBrowser(rawURL string) error {
	...
	switch runtime.GOOS {
	case "darwin":
		cmd = exec.Command("open", validated) //nolint:gosec
	case "linux":
		cmd = exec.Command("xdg-open", validated) //nolint:gosec
	case "windows":
		cmd = exec.Command("rundll32", "url.dll,FileProtocolHandler", validated) //nolint:gosec
	default:
...
func getTokenCachePath() string {
	...
	return filepath.Join(configDir, "bounceadmin", "token.json")
}
