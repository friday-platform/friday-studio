...
<dialog bind:this={dialog} class="diagnostics-dialog" onclick={handleBackdropClick}>
	...
	<div class="dialog-content" onclick={(e) => e.stopPropagation()}>
		...
		<div class="icon" class:success={status === 'success'} class:error={status === 'error'}>
			...
			{:else}
				<svg width="32" height="32" viewBox="0 0 24 24" fill="none">
					<path
						d="M9 11L12 14L22 4M21 12V19A2 2 0 0119 21H5A2 2 0 013 19V5A2 2 0 015 3H16"
						stroke="currentColor"
						stroke-width="2"
						stroke-linecap="round"
						stroke-linejoin="round"
		...
		{#if status === 'error'}
			<div class="actions">
				<button class="retry-button" onclick={runDiagnostics}> Retry </button>
				<button class="close-button-text" onclick={handleClose}> Close </button>
			</div>
		{/if}
	</div>
</dialog>
