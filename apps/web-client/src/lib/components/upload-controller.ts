import { uploadFile, validateFile, type UploadStatus } from "../utils/upload.ts";
 ...
 * The Svelte component syncs `$state` from the controller via the `onUpdate` callback.
 */
export function createUploadController(opts: UploadControllerOptions) {
  ...
  function handleFile(selectedFile: File) {
    // Re-entrancy guard: abort in-flight upload before starting new one
    if (_abortController) {
    ...
    uploadFile(
      ...
      .then((result: { artifactId: string } | { error: string }) => {
        // Staleness check: if controller changed, this promise is from a previous upload
  ...
  function destroy() {
    ...
    // Only clear parent formData if upload didn't complete successfully
    if (_status !== "ready") {
      opts.onchange(undefined);
    }
  }
