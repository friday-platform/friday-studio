...
import { type OutputEntry, SSEEventSchema } from "$lib/modules/messages/types";
  ...
  eventSource.onmessage = (event) => {
    try {
      ...
      if (latestMessage?.type === "request") {
        typingState = { isTyping: true, elapsedSeconds: 0 };
      }

      if (latestMessage?.type === "finish" || latestMessage?.type === "error") {
        typingState = { isTyping: false, elapsedSeconds: 0 };
      }
