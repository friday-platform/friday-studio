...
import { setSessionDetailContext } from "./context.svelte";

let { data }: { data: PageData } = $props();

setSessionDetailContext(data.session);

const sessionDate = formatSessionDate(data.session.metadata.createdAt);
const timelineData = $derived(parseSessionTimeline(data.session.metadata, data.session.events));
</script>
...
<div class="page">
	<div class="content">
		<span
			...
			class:pending={data.session.metadata.status === 'pending'}
		>
			...
			{:else if data.session.metadata.status === 'failed'}
				<IconSmall.Close />
				Failed
			{:else if data.session.metadata.status === 'pending'}
				<IconSmall.Progress />
				In Progress
			{/if}
		</span>
		...
		<div class="details">
			...
			{#if data.session.metadata.artifactIds.length > 0}
				<h2>Artifacts</h2>

				<Artifacts artifacts={data.session.metadata.artifactIds} />
			{/if}
