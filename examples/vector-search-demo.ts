...
async function vectorSearchDemo() {
  ...
  memoryManager.rememberWithMetadata("auth-concept", {
    ...
    definition: "OAuth2 is an authorization framework that enables applications to obtain limited access to user accounts",
  ...
  authResults.forEach((result, i) => {
    console.log(`   ${i + 1}. [${result.memoryType}] ${result.id} (similarity: ${result.similarity.toFixed(3)})`);
