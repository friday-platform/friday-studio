...
export class EchoAgent implements BaseAgent {
  getMetadata(): Agent {
    return {
      name: "echo",
      description:
        "A simple echo agent that repeats and processes user input with helpful responses",
      metadata: {
        capabilities: [
          {
        ...
        documentation:
          "A demonstration agent that echoes user input with added processing and analysis.",
        license: "Apache-2.0",
      },
      status: {
        avg_run_tokens: 50,
...
export class ChatAgent implements BaseAgent {
  getMetadata(): Agent {
    return {
      ...
      metadata: {
        ...
        natural_languages: ["en"],
        documentation:
          "A demonstration chat agent that provides friendly, contextual responses to user messages.",
        license: "Apache-2.0",
      },
      status: {
        avg_run_tokens: 75,
  ...
  private generateChatResponse(input: string): string {
    ...
    if (lowerInput.includes("time")) {
      ...
      }. Is there something time-sensitive I can help you with?`;
    }
