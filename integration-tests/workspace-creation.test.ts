...
for (const scenario of testScenarios) {
  Deno.test(`Integration: ${scenario.name}`, async () => {
    ...
    if (jobCount > 0) {
      ...
      for (const [name, job] of Object.entries(validatedConfig.jobs || {})) {
        ...
        console.log(`     Triggers: [${job.triggers?.map((t) => t.signal).join(", ") || "none"}]`);
    ...
    console.log(`\n✅ === COMPONENT COUNT VALIDATION ===`);
    console.log(`🔔 Signals: ${signalCount} >= ${scenario.expectedComponents.minSignals} required`);
    ...
    for (const [jobName, job] of Object.entries(validatedConfig.jobs || {})) {
      ...
      for (const trigger of job.triggers || []) {
        ...
        if (!signalExists) {
          ...
          referenceErrors.push(`Job '${jobName}' references undefined signal '${trigger.signal}'`);
        }
      }
