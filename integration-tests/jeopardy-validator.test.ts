...
const logger = createLogger({ component: "JeopardyValidatorTest", test: true });
        ...
        doGenerate: () =>
          Promise.resolve({
            ...
            content: [{
              type: "text",
              text:
                `{"answersTask":true,"completeness":1,"confidence":1,"issues":[],"reasoning":""}`,
            }],
            warnings: [],
          }),
      }),
...
Deno.test(
  "jeopardy: combines structured issues with task-derived source checks; blocks on critical wrong_source",
  async () => {
    ...
    const req: JeopardyValidationRequest = {
      ...
      agentOutput:
        "Here are options: Hotel A - https://hotels.com/a, Hotel B - https://hotels.com/b",
      agentId: "agent-x",
    };

    const res: JeopardyValidationResult = await v.validate(req);

    // Expect critical wrong_source + structured incomplete
...
Deno.test(
  "jeopardy: respects task-derived allowedSources; isValid true when no critical issues",
  async () => {
    const v = createValidator({
      llmProvider: () =>
        new MockLanguageModelV2({
